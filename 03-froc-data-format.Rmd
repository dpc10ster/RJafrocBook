...
#### The `NL` worksheet organization
The following screen-shots show different parts of the `NL` worsheet for `dataset04`.

* The `NL`  worksheet lists the ratings of `NLs` on both non-diseased and diseased cases. __Unlike the ROC paradigm, `NLs` can occur on diseased cases. Additionally, the number of rows per case cannot be predicted apriori. It could be 0, 1, 2, etc. __
* It is possible that the `NL` worsheet is blank. See [@RN2680] for how the FROC paradigm correctly interprets this situation as indicative of a very good observer.
* 

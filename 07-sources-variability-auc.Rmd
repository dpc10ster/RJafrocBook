...
```{r sourcesVariabilitybs1, echo=FALSE}
Ktr = array(dim = c(2,5))
...
After setting `parametricFOM` to `TRUE` (for a parametric fit), `seed` to 1 and `P` to 2000, the ROC counts table is defined and the function `doCalSimulator()` is called. The returned `list` contains `OrigAUC`, the AUC of the original data, calculated by `RocfitR()`, in this case `OrigAUC` = `r ret$OrigAUC`, and the mean and standard deviation of the 2000 AUC values, are equal to `r ret$meanAUC` and `r ret$stdAUC`, respectively.
...
This time the mean and standard deviation of the 2000 AUC values, are equal to `r ret$meanAUC` and `r ret$stdAUC`, respectively. The respective values for the two values of `seed` are quite close to each other (to within a percent). 
...
For `seed` = 1 and `P` = 200 the mean and standard deviation of the 2000 AUC values, are equal to `r ret$meanAUC` and `r ret$stdAUC`, respectively. 
...
For `seed` = 2 and `P` = 200 the mean and standard deviation of the 2000 AUC values, are equal to `r ret$meanAUC` and `r ret$stdAUC`, respectively. Note the greater variability between the respective values.
...
P <- 2000
RocCountsTable = array(dim = c(2,5))
RocCountsTable[1,]  <- c(30,19,8,2,1) * 10
RocCountsTable[2,]  <- c(5,6,5,12,22) * 10
ret <- doCalSimulator(P, parametricFOM, RocCountsTable)
cat("parametricFOM = ", parametricFOM,
  "\nCalibrated simulator values:\n", 
    "\nmu = ", ret$mu, 
...
df <- array(dim = c(5,4))
df[1,] <- c("ExampleTable", "Rocfit", "MLE", "0.0378")
df[2,] <- c("ExampleTable", "Rocfit", "DeLong", "*0.0380")
df[3,] <- c("ExampleTable", "Rocfit", "Bootstrap", "0.0438")
df[4,] <- c("ExampleTable", "Rocfit", "Jackknife", "0.0386")
df[5,] <- c("ExampleTable", "Rocfit", "PopulationSampling", "0.0403")
df <- as.data.frame(df)
colnames(df) <- c("Dataset", "AUC est. method", "Var. est. method", "std. dev. AUC")
knitr::kable(df, caption = "Different estimates of standard deviation of AUC (* the entry for the DeLong method was obtained using the empirical AUC).", escape = FALSE)
```
...
df[2,] <- c("ExampleTable", "Wilcoxon", "DeLong", "0.0380")
df[3,] <- c("ExampleTable", "Wilcoxon", "Bootstrap", "0.0413")
df[4,] <- c("ExampleTable", "Wilcoxon", "Jackknife", "0.0369")
df[5,] <- c("ExampleTable", "Wilcoxon", "PopulationSampling", "0.0369")
df <- as.data.frame(df)
colnames(df) <- c("Dataset", "AUC est. method", "Var. est. method", "std. dev. AUC")

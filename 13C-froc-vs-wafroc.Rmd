...
```{r froc-vs-wafroc-plot2, fig.cap="Similar to preceding figure but with the following changes. All RAD_2 curves are for $\\mu = 2$ and for panels A, B and C $\\zeta_1 = 2$ for RAD-2.", fig.show='hold', echo=FALSE}
...
```{r froc-vs-wafroc-plot3, fig.cap="Similar to preceding figure but with the following changes. All RAD_2 curves are for $\\mu = 1.1$ and for panels A, B and C, $\\zeta_1 = -1$ for RAD-2.", fig.show='hold', echo=FALSE}
...
Consider the wAFROC AUCs for the RAD-2 curves in Fig. \@ref(fig:froc-vs-wafroc-plot2) panels B and E. The wAFROC AUC for RAD-2 in panel B is `r wafroc_2_2B` while that for RAD-2 in panel E is `r wafroc_2_2E`. The only difference between the simulation parameters for the two curves are $\zeta_1 = 2$ for panel B and $\zeta_1 = -\infty$ for panel E. Clearly wAFROC AUC depends on the value of $\zeta_1$. 

A similar result applies when considering the ROC curves in Fig. \@ref(fig:froc-vs-wafroc-plot2) panels C and F. The ROC AUC for RAD-2 in panel C is `r roc_2_2C` while that for RAD-2 in panel F is `r roc_2_2F`. Clearly ROC AUC also depends on the value of $\zeta_1$. 

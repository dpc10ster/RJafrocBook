...
## Formulae for statistical power calculation, RRRC
In Chapter [Dorfman Berbaum Metz Hillis (DBMH) Analysis] attention was on the NH distribution of the F-statistic. If the AH is true, i.e., if $\sigma_{\tau}^{2} \neq 0$, then the following holds, Eqn. (9.17):	
...
The non-central F-distribution will be used later for sample size estimation.

## Formulae for statistical power calculation, FRRC
...
## Background on central and non-central F-distributions

...
Alternatively, as with \@ref(eq:F1RMT),

$$(I-1)F_{ORH \mid R} \sim t_{I-1}$$
For $J$ = 1, \@ref(eq:DefFStatFRRC) reduces to \@ref(eq:DefF1RMT). 
...
\begin{equation}
CI_{1-\alpha}=(\theta_{i \bullet} - \theta_{i' \bullet}) \pm t_{\alpha/2, \infty}\sqrt{\frac{2}{J}(Var-Cov_1+(J-1)\max(Cov_2-Cov_3,0))}
(\#eq:CIalphaFRRC)
\end{equation}

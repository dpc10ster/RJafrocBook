...
\tightlist
\item
  There are 100 diseased cases (labeled 0-99) under column \texttt{CaseID} and 100 non-diseased cases (labeled 109-199). \footnote{The non-diseased cases numbered 128 - 199 are not shown above. They are similar to the ones that are shown - one row per case with a zero under the lesionID column and a zero under the Weights column.}\\
\item
...
\item
  As an example, there are two rows for \texttt{CaseID} = 77: one with \texttt{LesionID} = 1 (labeling the first lesion on this case) and one with \texttt{LesionID} = 2 (labeling the second and last lesion on this case). The weights of these lesions are explicitly specified to 0.5 each.
\item
...
\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-FP-1.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-FP-2.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-FP-3.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-FP-4.png}

\begin{itemize}
\item
  The \texttt{FP} worksheet lists the ratings of \texttt{NLs} on both non-diseased and diseased cases. \textbf{Unlike the ROC paradigm, \texttt{NLs} can occur on diseased cases. Additionally, the number of rows per case cannot be predicted apriori. It could be 0, 1, 2, etc. }
\item
  It is possible that the \texttt{FP} worsheet is blank. See \citep{RN2680} for how the FROC paradigm correctly interprets this situation as indicative of good performance.
\item
\end{itemize}

\hypertarget{the-tpll-worksheet-organization-1}{%
\subsubsection{\texorpdfstring{The \texttt{TP/LL} worksheet organization}{The TP/LL worksheet organization}}\label{the-tpll-worksheet-organization-1}}

The following screen-shots show different parts of the \texttt{TP} worsheet for \texttt{dataset04}.

\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-TP-1.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-TP-2.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-TP-3.png}\includegraphics[width=0.4\textwidth,height=\textheight]{images/FROC-TP-4.png}

\begin{itemize}
\tightlist
\item
  The \texttt{TP} worksheet lists the ratings of \texttt{LLs} on diseased cases. \textbf{Unlike the ROC paradigm, \texttt{NLs} can occur on diseased cases. Additionally, the number of rows per case cannot be predicted apriori. It could be 0, 1, 2, etc. }
\item
  It is possible (but unlikely) that the \texttt{TP} worsheet is blank. See \citep{RN2680} for how the FROC paradigm correctly interprets this situation as indicative of poor performance.
\end{itemize}

...
\CommentTok{\#\textgreater{} 1    1.4291399          1   0.23190486}
\end{Highlighting}
\end{Shaded}
...
The critical value of the F-statistic for rejection of the null hypothesis is \(F_{1-\alpha,\text{ndf},\text{ddf}_H}\), i.e., that value such that fraction \((1-\alpha)\) of the area under the distribution lies to the left of the critical value. From definition Eqn. \eqref{eq:F-ORH-RRRC}:
...
The NH is rejected if any of the following equivalent conditions is met (these statements are also true for \texttt{RRRC} analysis, and the \texttt{RRFC} analysis to be described next):
...
\hypertarget{ORExamples}{%
\chapter{Illustration of the OR method}\label{ORExamples}}

\hypertarget{ORExamples-introduction}{%
\section{Introduction}\label{ORExamples-introduction}}
...
\item
  The first example uses a two-treatment five-reader \textbf{ROC} dataset, a well-know and widely used dataset in the literature \citep{RN1993}.
\item
  The second example uses a five-treatment four-reader \textbf{ROC} dataset, showing the effect of multiple treatment pairings.
\item
  The third example uses a five-treatment four-reader \textbf{FROC} dataset, showing the key difference involved in FROC analysis, namely the choice of figure of merit.
\item
  The fourth example uses a five-treatment four-reader \textbf{FROC} dataset, showing the key difference involved in DBM vs.~OR analysis.
\item
...
\hypertarget{ORExamples-dataset02}{%
\section{Illustration with dataset02}\label{ORExamples-dataset02}}

\begin{itemize}
\tightlist
\item
  \texttt{dataset02} has two modalities and five readers.
\end{itemize}
...
\CommentTok{\#\textgreater{} I =  2 , J =  5}
\end{Highlighting}
\end{Shaded}

\hypertarget{ORExamples-RRRC-dataset02}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset02}}
...
\item
  The following code uses \texttt{UtilFigureOfMerit()} to this end. Note that \texttt{FOM} has to be explicitly specified as ``Wilcoxon''.
\end{itemize}
...
\tightlist
\item
  For example, for the first treatment, \texttt{trt0}, the second reader's figure of merit is 0.8587762.
\item
  The next step is to calculate the variance-covariance matrix and the mean-squares.
\item
  The function \texttt{UtilORVarComponentsFactorial()} returns these quantities. The \texttt{Factorial} in the function name is due to the fact that this code applies to the factorial design. A different function is used for a split-plot design.
\item
...
\KeywordTok{print}\NormalTok{(ret}\OperatorTok{$}\NormalTok{TRanova)}
...
\KeywordTok{print}\NormalTok{(ret}\OperatorTok{$}\NormalTok{VarCom)}
...
\item
  \texttt{ret} contains the values needed in this equation, as follows:
  ...
  \item
    \texttt{J}, the number of readers, is the length of the second dimension of \texttt{dataset02\$ratings\$NL}, i.e., 5.
  \item
    \texttt{Cov2} is in \texttt{ret\$VarCom{[}"Cov2",\ "Estimates"{]}}, whose value is \ensuremath{3.4407483\times 10^{-4}}.
  \item
    \texttt{Cov3} is in \texttt{ret\$VarCom{[}"Cov3",\ "Estimates"{]}}, whose value is \ensuremath{2.3902837\times 10^{-4}}.
  \end{itemize}
\end{itemize}

Applying Eqn. \eqref{eq:F-ORH-RRRC} one gets (\texttt{den} is the denominator on the right hand side of the referenced equation):
...
\NormalTok{F\_ORH\_RRRC \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{TRanova[}\StringTok{"T"}\NormalTok{, }\StringTok{"MS"}\NormalTok{]}\OperatorTok{/}\NormalTok{den}
...
\item
  From the previous chapter, \texttt{ddf} is calculated using Eqn. \eqref{eq:ddfH-RRRC}). The numerator of \texttt{ddf} is seen to be identical to \texttt{den\^{}2}, where \texttt{den} was calculated in the preceding code block. The implementation follows:
\end{itemize}
...
\tightlist
\item
  From the previous chapter, the \((1-\alpha)\) confidence interval for \(\theta_{1 \bullet} - \theta_{2 \bullet}\) is given by Eqn. \eqref{eq:CI-RRRC}.
\item
...
\NormalTok{CI\_RRRC \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(trtMeanDiffs }\OperatorTok{{-}}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr, trtMeanDiffs }\OperatorTok{+}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr)}
\KeywordTok{names}\NormalTok{(CI\_RRRC) \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Lower"}\NormalTok{, }\StringTok{"Right"}\NormalTok{)}
...
\item
  Finally, the preceding results are next compared to \texttt{RJafroc}.
\item
  This is accomplished using the significance testing function \texttt{StSignificanceTesting()}.
\item
  This function takes as arguments the \texttt{dataset}, the \texttt{FOM}, set to ``Wilcoxon'', the method of analysis, set to ``OR'' and \texttt{analysisOption}, set to ``RRRC''. The significance level of the test, also an argument, \texttt{alpha}, defaults to 0.05.
\item
  The code below applies this function and saves the returned object to \texttt{st1}.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{st1 \textless{}{-}}\StringTok{ }\KeywordTok{StSignificanceTesting}\NormalTok{(dataset02, }\DataTypeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\DataTypeTok{method =} \StringTok{"OR"}\NormalTok{, }\DataTypeTok{analysisOption =} \StringTok{"RRRC"}\NormalTok{)}
...
\item
  These are contained in the \texttt{ANOVA} \texttt{list} object.
\item
  \texttt{ANOVA\$TRanova} contains the treatment-reader ANOVA table, i.e.~the sum of squares, the degrees of freedom and the mean-squares, listed for the treatment, reader and treatment-reader factors, i.e., \texttt{T}, \texttt{R} and \texttt{TR}.
\item
...
\tightlist
\item
  Displayed next are the results of RRRC analysis, contained in the \texttt{RRRC} \texttt{list} object.
\item
  \texttt{RRRC\$FTests} contains the results of the F-tests.
\item
  \texttt{RRRC\$ciDiffTrt} contains the results of the confidence intervals for the inter-treatment difference FOMs, averaged over readers.
\item
  \texttt{RRRC\$ciAvgRdrEachTrt} contains the results of the confidence intervals for the treatments, averaged over readers.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{RRRC)}
...
\CommentTok{\#\textgreater{} trt0{-}trt1 {-}0.043800322 0.020748618 15.259675 {-}2.1109995 0.051665686}
\CommentTok{\#\textgreater{}                CILower       CIUpper}
\CommentTok{\#\textgreater{} trt0{-}trt1 {-}0.087959499 0.00035885444}
\CommentTok{\#\textgreater{} }
...
\hypertarget{ORExamples-FRRC-dataset02}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset02}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{st1 \textless{}{-}}\StringTok{ }\KeywordTok{StSignificanceTesting}\NormalTok{(dataset02, }\DataTypeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\DataTypeTok{method =} \StringTok{"OR"}\NormalTok{, }\DataTypeTok{analysisOption =} \StringTok{"FRRC"}\NormalTok{)}
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{FRRC)}
\CommentTok{\#\textgreater{} $FTests}
...
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $ciDiffTrtEachRdr}
\CommentTok{\#\textgreater{}                     Estimate      StdErr           z       PrGTz      CILower}
...
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $IndividualRdrVarCov1}
\CommentTok{\#\textgreater{}         varEachRdr   cov1EachRdr}
\CommentTok{\#\textgreater{} rdr0 0.00069890056 3.7346610e{-}04}
\CommentTok{\#\textgreater{} rdr1 0.00110605283 7.6015978e{-}04}
\CommentTok{\#\textgreater{} rdr2 0.00084234345 3.5532241e{-}04}
\CommentTok{\#\textgreater{} rdr3 0.00015057773 1.0833995e{-}06}
\CommentTok{\#\textgreater{} rdr4 0.00121356676 2.4303685e{-}04}
\end{Highlighting}
\end{Shaded}

\hypertarget{ORExamples-RRFC-dataset02}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset02}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{st1 \textless{}{-}}\StringTok{ }\KeywordTok{StSignificanceTesting}\NormalTok{(dataset02, }\DataTypeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\DataTypeTok{method =} \StringTok{"OR"}\NormalTok{, }\DataTypeTok{analysisOption =} \StringTok{"RRFC"}\NormalTok{)}
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{RRFC)}
\CommentTok{\#\textgreater{} $FTests}
...
\hypertarget{ORExamples-dataset04}{%
\section{Illustration with dataset04}\label{ORExamples-dataset04}}

\begin{itemize}
\tightlist
\item
  The second example uses the Federica Zanca dataset \citep{RN1882}, i.e., \texttt{dataset04}, which has five modalities and four readers.
\item
...
\item
  These are contained in the returned \texttt{list} object \texttt{st2}.
\end{itemize}
...
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FOMs)}
\CommentTok{\#\textgreater{} $foms}
...
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{TRanova)}
\CommentTok{\#\textgreater{}              SS DF            MS}
...
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{VarCom)}
\CommentTok{\#\textgreater{}            Estimates       Rhos}
...
\hypertarget{ORExamples-RRRC-dataset04}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset04}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}                  DF            MS     FStat           p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}             Estimate      StdErr        DF            t       PrGTt}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}       Estimate      StdErr         DF    CILower    CIUpper          Cov2}
...
\hypertarget{ORExamples-FRRC-dataset04}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset04}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}                     MS     Chisq DF            p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}             Estimate      StdErr            z       PrGTz       CILower}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}       Estimate      StdErr  DF    CILower    CIUpper}
...
\hypertarget{ORExamples-RRFC-dataset04}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset04}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRFC)}
\CommentTok{\#\textgreater{} $FTests}
\CommentTok{\#\textgreater{}    DF            MS         F           p}
...
\hypertarget{ORExamples-dataset04-FROC}{%
\section{Illustration with dataset04, FROC analysis}\label{ORExamples-dataset04-FROC}}

\begin{itemize}
\tightlist
\item
  The third example uses \texttt{dataset04}, but this time we use the FROC data, i.e, we do not convert it to inferred-ROC.
\item
...
\NormalTok{st3 \textless{}{-}}\StringTok{ }\KeywordTok{StSignificanceTesting}\NormalTok{(ds, }\DataTypeTok{FOM =} \StringTok{"wAFROC"}\NormalTok{, }\DataTypeTok{method =} \StringTok{"OR"}\NormalTok{)}
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FOMs)}
\CommentTok{\#\textgreater{} $foms}
...
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{TRanova)}
\CommentTok{\#\textgreater{}              SS DF            MS}
...
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{VarCom)}
\CommentTok{\#\textgreater{}            Estimates       Rhos}
...
\hypertarget{ORExamples-RRRC-dataset04-FROC}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset04-FROC}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}                  DF            MS     FStat           p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}                Estimate     StdErr        DF           t         PrGTt}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}        Estimate      StdErr         DF    CILower    CIUpper          Cov2}
...
\hypertarget{ORExamples-FRRC-dataset04-FROC}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset04-FROC}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}                      MS     Chisq DF            p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}                Estimate      StdErr           z        PrGTz       CILower}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}        Estimate      StdErr  DF    CILower    CIUpper}
...
\hypertarget{ORExamples-RRFC-dataset04-FROC}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset04-FROC}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRFC)}
\CommentTok{\#\textgreater{} $FTests}
\CommentTok{\#\textgreater{}    DF            MS         F             p}
...
\hypertarget{ORExamples-dataset04-FROC-DBM}{%
\section{Illustration with dataset04, FROC analysis, DBM significance testing}\label{ORExamples-dataset04-FROC-DBM}}
...
\begin{Highlighting}[]
\NormalTok{ds \textless{}{-}}\StringTok{ }\NormalTok{dataset04 }\CommentTok{\# do NOT convert to ROC}
\NormalTok{st4 \textless{}{-}}\StringTok{ }\KeywordTok{StSignificanceTesting}\NormalTok{(ds, }\DataTypeTok{FOM =} \StringTok{"wAFROC"}\NormalTok{, }\DataTypeTok{method =} \StringTok{"DBM"}\NormalTok{) }\CommentTok{\# Note we are using DBM analysis}
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{FOMs)}
\CommentTok{\#\textgreater{} $foms}
\CommentTok{\#\textgreater{}            rdr1       rdr3       rdr4       rdr5}
\CommentTok{\#\textgreater{} trt1 0.77926667 0.72489167 0.70362500 0.80509167}
\CommentTok{\#\textgreater{} trt2 0.78700000 0.72690000 0.72261667 0.80378333}
...
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0082291667}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0548791667}
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{TRCanova)}
\CommentTok{\#\textgreater{}                 SS   DF          MS}
...
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{ANOVA}\OperatorTok{$}\NormalTok{VarCom)}
...
\hypertarget{ORExamples-RRRC-dataset04-FROC-DBM}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset04-FROC-DBM}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}                  DF          MS     FStat           p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}                Estimate     StdErr        DF           t         PrGTt}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0068562500 0.01218567 36.793343 {-}0.56264860 5.7708666e{-}01}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0306145833 0.01218567 36.793343  2.51234312 1.6509287e{-}02}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0160354167 0.01218567 36.793343 {-}1.31592413 1.9633987e{-}01}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}        Estimate      StdErr         DF    CILower    CIUpper}
...
\hypertarget{ORExamples-FRRC-dataset04-FROC-DBM}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset04-FROC-DBM}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}            DF         MS     FStat           p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt)}
\CommentTok{\#\textgreater{}                Estimate      StdErr  DF           t        PrGTt       CILower}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt)}
\CommentTok{\#\textgreater{}        Estimate      StdErr  DF    CILower    CIUpper}
...
\hypertarget{ORExamples-RRFC-dataset04-FROC-DBM}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset04-FROC-DBM}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st4}\OperatorTok{$}\NormalTok{RRFC)}
\CommentTok{\#\textgreater{} $FTests}
\CommentTok{\#\textgreater{}           DF          MS     FStat             p}
...
\hypertarget{ORExamples-references}{%
\section{References}\label{ORExamples-references}}

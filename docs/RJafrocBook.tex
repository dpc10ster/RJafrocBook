...
\CommentTok{\#\textgreater{} 1    1.0778308          1   0.29918334}
\end{Highlighting}
\end{Shaded}
...
\chapter{Illustrations using the Obuchowski Rockette method}\label{ORExamples}}
...
\tightlist
\item
  Four examples are given. The first is a hand-calculation (i.e., from first principles, using the relevant formulae in the previous chapter). The next three examples use OR analysis as implemented in the \texttt{RJafroc} package.
\item
  The first example uses a two-treatment five-reader \textbf{ROC} dataset, well-know in the literature \citep{RN1993}, as it has been used to illustrate advances in ROC methodology.
\item
  The second example uses a five-treatment four-reader \textbf{ROC} dataset, showing the effect of multiple treatment pairings on the significance testing (the first dataset has only one treatment pairing).
\item
  The third example uses a five-treatment four-reader \textbf{FROC} dataset, showing the key difference involved in FROC analysis.
\item
...
\hypertarget{ORExamples-dataset02-hand}{%
\section{Hand calculation using dataset02}\label{ORExamples-dataset02-hand}}

\begin{itemize}
\tightlist
\item
  \texttt{dataset02} has 2 modalities and 5 readers.
\end{itemize}
...
\item
  The following code uses \texttt{UtilFigureOfMerit()} to this end. Note that \texttt{FOM} is explicitly specified as ``Wilcoxon''.
\end{itemize}
...
\item
  The function \texttt{UtilORVarComponentsFactorial()} returns these quantities, which are saved to \texttt{vc}. The \texttt{Factorial} in the function name is due to the fact that this code applies to the factorial design. A different function is used for a split-plot design.
\item
  Only the first two members of the returned object are displayed in full. The remaining members, \texttt{vc\$IndividualTrt} and \texttt{vc\$IndividualRdr}, are used in single-treatment and single-reader analyses.
\end{itemize}
...
\NormalTok{  dataset02, }
  \DataTypeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\DataTypeTok{covEstMethod =} \StringTok{"jackknife"}\NormalTok{)}
\KeywordTok{summary}\NormalTok{(vc)}
\CommentTok{\#\textgreater{}               Length Class      Mode}
\CommentTok{\#\textgreater{} TRanova       3      data.frame list}
\CommentTok{\#\textgreater{} VarCom        2      data.frame list}
\CommentTok{\#\textgreater{} IndividualTrt 4      data.frame list}
\CommentTok{\#\textgreater{} IndividualRdr 4      data.frame list}
\KeywordTok{print}\NormalTok{(vc}\OperatorTok{$}\NormalTok{TRanova)}
...
\KeywordTok{print}\NormalTok{(vc}\OperatorTok{$}\NormalTok{VarCom)}
...
\hypertarget{ORExamples-RRRC-dataset02-hand}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset02-hand}}

\begin{itemize}
\tightlist
\item
  The next step is the calculate the NH testing statistic. The relevant equation is Eqn. \eqref{eq:F-ORH-RRRC}. \texttt{vc} contains the values needed in this equation, as follows:
\item
  MS(T) is contained in \texttt{vc\$TRanova{[}"T",\ "MS"{]}}, whose value is 0.0047962.
\item
  MS(TR) is contained in \texttt{vc\$TRanova{[}"TR",\ "MS"{]}}, whose value is \ensuremath{5.5103062\times 10^{-4}}.
\item
  \texttt{Cov2} is contained in \texttt{vc\$VarCom{[}"Cov2",\ "Estimates"{]}}, whose value is \ensuremath{3.4407483\times 10^{-4}}.
\item
  \texttt{Cov3} is contained in \texttt{vc\$VarCom{[}"Cov3",\ "Estimates"{]}}, whose value is \ensuremath{2.3902837\times 10^{-4}}.
\end{itemize}

{[}\texttt{Cov1}, contained in \texttt{vc\$VarCom{[}"Cov2",\ "Estimates"{]}} is not needed in the current calculation.{]}
...
\item
  The denominator degrees of freedom, \texttt{ddf}, is calculated using Eqn. \eqref{eq:ddfH-RRRC}).
\item
  The numerator of \texttt{ddf} is seen to be identical to \texttt{den\^{}2}, where \texttt{den} was calculated in the preceding code block. The implementation follows:
\end{itemize}
...
\tightlist
\item
  The value of \texttt{ddf} is 15.2596746.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p\_RRRC \textless{}{-}}\StringTok{ }\DecValTok{1} \OperatorTok{{-}}\StringTok{ }\KeywordTok{pf}\NormalTok{(F\_ORH\_RRRC, I }\OperatorTok{{-}}\StringTok{ }\DecValTok{1}\NormalTok{, ddf)}
\end{Highlighting}
\end{Shaded}
...
\item
  The next step is calculation of the confidence interval for paired FOM differences. Since \texttt{I} = 2, their is only one paired difference in reader-averaged FOMs, namely, the first treatment minus the second:
\end{itemize}
...
\item
  The \((1-\alpha)\) confidence interval the difference is given by Eqn. \eqref{eq:CI-RRRC}.
\item
  The expression inside the square-root symbol in this equation is \texttt{2/J*den}.
\item
...
\hypertarget{ORExamples-FRRC-dataset02-hand}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset02-hand}}

\begin{itemize}
\tightlist
\item
  The F-statistic for testing the NH of no treatment effect was stated in \eqref{eq:DefFStatFRRC-OR}.
\item
  The quantities needed to evaluate the right hand side are:
\item
  \texttt{MS(T)}, which is contained in vc\$TRanova{[}``T'', ``MS''{]};
\item
  \texttt{Var}, which is contained in vc\$VarCom{[}``Var'', ``Estimates''{]};
\item
  \texttt{Cov1}, which is contained in vc\$VarCom{[}``Cov1'', ``Estimates''{]};
\item
  \texttt{Cov2}, which is contained in vc\$VarCom{[}``Cov2'', ``Estimates''{]};
\item
  \texttt{Cov3}, which is contained in vc\$VarCom{[}``Cov3'', ``Estimates''{]}.
\item
  The implementation follows:
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{den \textless{}{-}}\StringTok{ }\NormalTok{vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Var"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{] }\OperatorTok{{-}}\StringTok{ }
...
\StringTok{  }\NormalTok{(J}\DecValTok{{-}1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\KeywordTok{max}\NormalTok{(vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Cov2"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{]}\OperatorTok{{-}}\NormalTok{vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Cov3"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{],}\DecValTok{0}\NormalTok{)}
\NormalTok{F\_ORH\_FRRC \textless{}{-}}\StringTok{ }\NormalTok{vc}\OperatorTok{$}\NormalTok{TRanova[}\StringTok{"T"}\NormalTok{, }\StringTok{"MS"}\NormalTok{]}\OperatorTok{/}\NormalTok{den}
\KeywordTok{print}\NormalTok{(F\_ORH\_FRRC)}
\CommentTok{\#\textgreater{} [1] 5.475953}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{F\_ORH\_FRRC} is distributed as an F-statistic with \texttt{ndf} = I-1 and \texttt{ddf} = infinity.
\item
  \texttt{F\_ORH\_FRRC} times (I-1) is distributed as a chi-square-statistic with \(I-1\) degrees of freedom.
\item
  The observed value of the chi-square statistic is 5.4759532.
\item
  The p-value is calculated using \eqref{eq:pValueFRRC-OR}, whose implementation follows:
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p\_FRRC \textless{}{-}}\StringTok{ }\DecValTok{1} \OperatorTok{{-}}\StringTok{ }\KeywordTok{pchisq}\NormalTok{(F\_ORH\_FRRC }\OperatorTok{*}\StringTok{ }\NormalTok{(I }\OperatorTok{{-}}\StringTok{ }\DecValTok{1}\NormalTok{), I }\OperatorTok{{-}}\StringTok{ }\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  The confidence interval for the difference FOM is in Eqn. \eqref{eq:CIDiffFomFRRC-OR}. Its implementation follows:
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stdErr \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{2}\OperatorTok{/}\NormalTok{J }\OperatorTok{*}\StringTok{ }\NormalTok{(vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Var"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{] }\OperatorTok{{-}}\StringTok{ }
\StringTok{                        }\NormalTok{vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Cov1"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{] }\OperatorTok{+}\StringTok{ }
\StringTok{                        }\NormalTok{(J}\DecValTok{{-}1}\NormalTok{)}\OperatorTok{*}\KeywordTok{max}\NormalTok{(vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Cov2"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{]}\OperatorTok{{-}}\NormalTok{vc}\OperatorTok{$}\NormalTok{VarCom[}\StringTok{"Cov3"}\NormalTok{, }\StringTok{"Estimates"}\NormalTok{],}\DecValTok{0}\NormalTok{)))}
\NormalTok{t\_crit \textless{}{-}}\StringTok{ }\KeywordTok{abs}\NormalTok{(}\KeywordTok{qt}\NormalTok{(alpha}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\OtherTok{Inf}\NormalTok{))}
\NormalTok{CI\_FRRC \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(trtMeanDiffs }\OperatorTok{{-}}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr, trtMeanDiffs }\OperatorTok{+}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr)}
\KeywordTok{names}\NormalTok{(CI\_FRRC) \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Lower"}\NormalTok{, }\StringTok{"Upper"}\NormalTok{)}
\KeywordTok{print}\NormalTok{(CI\_FRRC)}
\CommentTok{\#\textgreater{}       Lower       Upper }
\CommentTok{\#\textgreater{} {-}0.08048591 {-}0.00711473}
\end{Highlighting}
\end{Shaded}

Note that this time the confidence interval does not include zero, consistent with the p-value finding.

One can calculate confidence intervals for each treatment, using data from that treatment only, Eqn. \eqref{eq:CIIndTrtFomFRRC-OR}. The required \texttt{Var\_i} and \texttt{Cov2\_i}, calculated over each treatment, are in \texttt{vc\$IndividualTrt}. The implementation follows.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{CI \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(I,}\DecValTok{2}\NormalTok{))}
\NormalTok{ci \textless{}{-}}\StringTok{ }\KeywordTok{data.frame}\NormalTok{()}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{I) \{}
\NormalTok{  stdErr \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{1}\OperatorTok{/}\NormalTok{J }\OperatorTok{*}\StringTok{ }\NormalTok{(vc}\OperatorTok{$}\NormalTok{IndividualTrt[i, }\StringTok{"varEachTrt"}\NormalTok{] }\OperatorTok{+}\StringTok{ }
\StringTok{                          }\NormalTok{(J}\DecValTok{{-}1}\NormalTok{)}\OperatorTok{*}\KeywordTok{max}\NormalTok{(vc}\OperatorTok{$}\NormalTok{IndividualTrt[i, }\StringTok{"cov2EachTrt"}\NormalTok{],}\DecValTok{0}\NormalTok{)))}
\NormalTok{  t\_crit \textless{}{-}}\StringTok{ }\KeywordTok{abs}\NormalTok{(}\KeywordTok{qt}\NormalTok{(alpha}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\OtherTok{Inf}\NormalTok{))}
\NormalTok{  CI[i,] \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(trtMeans[i] }\OperatorTok{{-}}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr, trtMeans[i] }\OperatorTok{+}\StringTok{ }\NormalTok{t\_crit}\OperatorTok{*}\NormalTok{stdErr)}
\NormalTok{  rowName \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"trt"}\NormalTok{, modalityID[i])}
\NormalTok{  ci \textless{}{-}}\StringTok{ }\KeywordTok{rbind}\NormalTok{(ci, }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{Estimate =}\NormalTok{ trtMeans[i], }
...
\CommentTok{\#\textgreater{} trt0 0.8970370 0.8494301 0.9446440}
\CommentTok{\#\textgreater{} trt1 0.9408374 0.9079564 0.9737183}
\end{Highlighting}
\end{Shaded}

One can calculate confidence intervals for inter-treatment FOM differences for individual readers. Each analysis is based only on data for the specified reader, i.e, on the reader-specific \texttt{AUC}, variance and \texttt{Cov1} estimates. The required \texttt{Var\_j} and \texttt{Cov1\_j} are in \texttt{vc\$IndividualRdr}. The implementation follows.
...
\NormalTok{    diffTRName[ii] \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"trt"}\NormalTok{, modalityID[i], }\DataTypeTok{sep =} \StringTok{"{-}"}\NormalTok{, }\StringTok{"trt"}\NormalTok{, modalityID[ip]) }\CommentTok{\# !sic}
\NormalTok{    ii \textless{}{-}}\StringTok{ }\NormalTok{ii }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\NormalTok{  \}}
\NormalTok{\}}

\NormalTok{trtMeanDiffs \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(J, }\KeywordTok{choose}\NormalTok{(I, }\DecValTok{2}\NormalTok{)))}
...
\NormalTok{  stdErr[j,] \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{(vc}\OperatorTok{$}\NormalTok{IndividualRdr[j,}\StringTok{"varEachRdr"}\NormalTok{] }\OperatorTok{{-}}\StringTok{ }\NormalTok{vc}\OperatorTok{$}\NormalTok{IndividualRdr[j,}\StringTok{"cov1EachRdr"}\NormalTok{]))}
\NormalTok{  pair \textless{}{-}}\StringTok{ }\DecValTok{1}
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{I) \{}
    ...
    \ControlFlowTok{for}\NormalTok{ (ip }\ControlFlowTok{in}\NormalTok{ (i }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)}\OperatorTok{:}\NormalTok{I) \{}
\NormalTok{      trtMeanDiffs[j, pair] \textless{}{-}}\StringTok{ }\NormalTok{foms[i, j] }\OperatorTok{{-}}\StringTok{ }\NormalTok{foms[ip, j]}
\NormalTok{      trDiffNames[j,pair] \textless{}{-}}\StringTok{ }\NormalTok{diffTRName[pair]}
\NormalTok{      zStat[j,pair] \textless{}{-}}\StringTok{ }\NormalTok{trtMeanDiffs[j,pair]}\OperatorTok{/}\NormalTok{stdErr[j,pair]}
\NormalTok{      PrGTz[j,pair] \textless{}{-}}\StringTok{ }\DecValTok{2} \OperatorTok{*}\StringTok{ }\KeywordTok{pnorm}\NormalTok{(}\KeywordTok{abs}\NormalTok{(zStat[j,pair]), }\DataTypeTok{lower.tail =} \OtherTok{FALSE}\NormalTok{)}
\NormalTok{      CIReader[j, pair,] \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(trtMeanDiffs[j,pair] }\OperatorTok{+}\StringTok{ }\KeywordTok{qnorm}\NormalTok{(alpha}\OperatorTok{/}\DecValTok{2}\NormalTok{) }\OperatorTok{*}\StringTok{ }\NormalTok{stdErr[j,pair], }
\NormalTok{                              trtMeanDiffs[j,pair] }\OperatorTok{+}\StringTok{ }\KeywordTok{qnorm}\NormalTok{(}\DecValTok{1}\OperatorTok{{-}}\NormalTok{alpha}\OperatorTok{/}\DecValTok{2}\NormalTok{) }\OperatorTok{*}\StringTok{ }\NormalTok{stdErr[j,pair])}
\NormalTok{      rowName \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"rdr"}\NormalTok{, Reader[j,pair], }\StringTok{"::"}\NormalTok{, trDiffNames[j, pair])}
\NormalTok{      ci \textless{}{-}}\StringTok{ }\KeywordTok{rbind}\NormalTok{(ci, }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{Estimate =}\NormalTok{ trtMeanDiffs[j, pair], }
...
\hypertarget{ORExamples-RRFC-dataset02-hand}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset02-hand}}

TBA

\hypertarget{ORExamples-dataset02-RJafroc}{%
\section{Using RJafroc: dataset02}\label{ORExamples-dataset02-RJafroc}}

\hypertarget{ORExamples-RRRC-dataset02-RJafroc}{%
\subsection{Random-Reader Random-Case (RRRC) analysis}\label{ORExamples-RRRC-dataset02-RJafroc}}
...
\item
  The significance level of the test, also an argument, \texttt{alpha}, defaults to 0.05.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{6}\NormalTok{)])}
\CommentTok{\#\textgreater{}        Estimate    CILower    CIUpper}
\CommentTok{\#\textgreater{} trt0 0.89703704 0.82522360 0.96885048}
\CommentTok{\#\textgreater{} trt1 0.94083736 0.89413783 0.98753689}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{ORExamples-FRRC-dataset02-RJafroc}{%
\subsection{Fixed-Reader Random-Case (FRRC) analysis}\label{ORExamples-FRRC-dataset02-RJafroc}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{4}\NormalTok{)])}
\CommentTok{\#\textgreater{}               Estimate      CILower       CIUpper}
\CommentTok{\#\textgreater{} trt0{-}trt1 {-}0.043800322 {-}0.080485914 {-}0.0071147303}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}        Estimate    CILower}
\CommentTok{\#\textgreater{} trt0 0.89703704 0.84943008}
\CommentTok{\#\textgreater{} trt1 0.94083736 0.90795637}
\end{Highlighting}
\end{Shaded}

\hypertarget{ORExamples-RRFC-dataset02-RJafroc}{%
\subsection{Random-Reader Fixed-Case (RRFC) analysis}\label{ORExamples-RRFC-dataset02-RJafroc}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st1}\OperatorTok{$}\NormalTok{RRFC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}               Estimate      CILower       CIUpper}
\CommentTok{\#\textgreater{} trt0{-}trt1 {-}0.043800322 {-}0.085020224 {-}0.0025804202}
\end{Highlighting}
\end{Shaded}

\hypertarget{ORExamples-dataset04-RJafroc}{%
\section{Using RJafroc: dataset04}\label{ORExamples-dataset04-RJafroc}}
...
\item
  The results are contained in the returned \texttt{list} object \texttt{st2}.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}             Estimate        CILower       CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0051000 {-}0.04002130451  0.0298213045}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0353250  0.00040369549  0.0702463045}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0054125 {-}0.04033380451  0.0295088045}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0367750  0.00185369549  0.0716963045}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0404250  0.00550369549  0.0753463045}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0003125 {-}0.03523380451  0.0346088045}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0418750  0.00695369549  0.0767963045}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0407375 {-}0.07565880451 {-}0.0058161955}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0014500 {-}0.03347130451  0.0363713045}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0421875  0.00726619549  0.0771088045}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{6}\NormalTok{)])}
\CommentTok{\#\textgreater{}       Estimate    CILower    CIUpper}
\CommentTok{\#\textgreater{} trt1 0.8451625 0.77351391 0.91681109}
\CommentTok{\#\textgreater{} trt2 0.8502625 0.80942311 0.89110189}
\CommentTok{\#\textgreater{} trt3 0.8098375 0.74689261 0.87278239}
\CommentTok{\#\textgreater{} trt4 0.8505750 0.77664342 0.92450658}
\CommentTok{\#\textgreater{} trt5 0.8083875 0.74706746 0.86970754}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{4}\NormalTok{)])}
\CommentTok{\#\textgreater{}             Estimate       CILower       CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0051000 {-}0.0378342386  0.0276342386}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0353250  0.0025907614  0.0680592386}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0054125 {-}0.0381467386  0.0273217386}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0367750  0.0040407614  0.0695092386}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0404250  0.0076907614  0.0731592386}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0003125 {-}0.0330467386  0.0324217386}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0418750  0.0091407614  0.0746092386}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0407375 {-}0.0734717386 {-}0.0080032614}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0014500 {-}0.0312842386  0.0341842386}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0421875  0.0094532614  0.0749217386}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}       Estimate    CILower}
\CommentTok{\#\textgreater{} trt1 0.8451625 0.80923466}
\CommentTok{\#\textgreater{} trt2 0.8502625 0.81167398}
\CommentTok{\#\textgreater{} trt3 0.8098375 0.77045178}
\CommentTok{\#\textgreater{} trt4 0.8505750 0.81406752}
\CommentTok{\#\textgreater{} trt5 0.8083875 0.76983500}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRFC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}    DF            MS         F           p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st2}\OperatorTok{$}\NormalTok{RRFC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}             Estimate       CILower       CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0051000 {-}0.0382225014  0.0280225014}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0353250  0.0022024986  0.0684475014}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0054125 {-}0.0385350014  0.0277100014}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0367750  0.0036524986  0.0698975014}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0404250  0.0073024986  0.0735475014}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0003125 {-}0.0334350014  0.0328100014}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0418750  0.0087524986  0.0749975014}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0407375 {-}0.0738600014 {-}0.0076149986}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0014500 {-}0.0316725014  0.0345725014}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0421875  0.0090649986  0.0753100014}
\end{Highlighting}
\end{Shaded}

\hypertarget{ORExamples-dataset04-FROC-RJafroc}{%
\section{Using RJafroc: dataset04, FROC analysis}\label{ORExamples-dataset04-FROC-RJafroc}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}                Estimate       CILower       CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0068562500 {-}0.0315514439  0.0178389439}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0306145833  0.0059193894  0.0553097773}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0160354167 {-}0.0407306106  0.0086597773}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0388437500  0.0141485561  0.0635389439}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0374708333  0.0127756394  0.0621660273}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0091791667 {-}0.0338743606  0.0155160273}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0457000000  0.0210048061  0.0703951939}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0466500000 {-}0.0713451939 {-}0.0219548061}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0082291667 {-}0.0164660273  0.0329243606}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0548791667  0.0301839727  0.0795743606}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{6}\NormalTok{)])}
\CommentTok{\#\textgreater{}        Estimate    CILower    CIUpper}
\CommentTok{\#\textgreater{} trt1 0.75321875 0.68413193 0.82230557}
\CommentTok{\#\textgreater{} trt2 0.76007500 0.69727167 0.82287833}
\CommentTok{\#\textgreater{} trt3 0.72260417 0.66128414 0.78392420}
\CommentTok{\#\textgreater{} trt4 0.76925417 0.67869638 0.85981195}
\CommentTok{\#\textgreater{} trt5 0.71437500 0.63473415 0.79401585}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{4}\NormalTok{)])}
\CommentTok{\#\textgreater{}                Estimate       CILower      CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0068562500 {-}0.0408485147  0.027136015}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0306145833 {-}0.0033776814  0.064606848}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0160354167 {-}0.0500276814  0.017956848}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0388437500  0.0048514853  0.072836015}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0374708333  0.0034785686  0.071463098}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0091791667 {-}0.0431714314  0.024813098}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0457000000  0.0117077353  0.079692265}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0466500000 {-}0.0806422647 {-}0.012657735}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0082291667 {-}0.0257630981  0.042221431}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0548791667  0.0208869019  0.088871431}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{FRRC}\OperatorTok{$}\NormalTok{ciAvgRdrEachTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}        Estimate    CILower}
\CommentTok{\#\textgreater{} trt1 0.75321875 0.71076166}
\CommentTok{\#\textgreater{} trt2 0.76007500 0.71538552}
\CommentTok{\#\textgreater{} trt3 0.72260417 0.68032336}
\CommentTok{\#\textgreater{} trt4 0.76925417 0.72777924}
\CommentTok{\#\textgreater{} trt5 0.71437500 0.66975007}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRFC}\OperatorTok{$}\NormalTok{FTests)}
\CommentTok{\#\textgreater{}    DF            MS         F             p}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(st3}\OperatorTok{$}\NormalTok{RRFC}\OperatorTok{$}\NormalTok{ciDiffTrt[,}\OperatorTok{{-}}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{5}\NormalTok{)])}
\CommentTok{\#\textgreater{}                Estimate      CILower       CIUpper}
\CommentTok{\#\textgreater{} trt1{-}trt2 {-}0.0068562500 {-}0.026920472  0.0132079720}
\CommentTok{\#\textgreater{} trt1{-}trt3  0.0306145833  0.010550361  0.0506788053}
\CommentTok{\#\textgreater{} trt1{-}trt4 {-}0.0160354167 {-}0.036099639  0.0040288053}
\CommentTok{\#\textgreater{} trt1{-}trt5  0.0388437500  0.018779528  0.0589079720}
\CommentTok{\#\textgreater{} trt2{-}trt3  0.0374708333  0.017406611  0.0575350553}
\CommentTok{\#\textgreater{} trt2{-}trt4 {-}0.0091791667 {-}0.029243389  0.0108850553}
\CommentTok{\#\textgreater{} trt2{-}trt5  0.0457000000  0.025635778  0.0657642220}
\CommentTok{\#\textgreater{} trt3{-}trt4 {-}0.0466500000 {-}0.066714222 {-}0.0265857780}
\CommentTok{\#\textgreater{} trt3{-}trt5  0.0082291667 {-}0.011835055  0.0282933886}
\CommentTok{\#\textgreater{} trt4{-}trt5  0.0548791667  0.034814945  0.0749433886}
\end{Highlighting}
\end{Shaded}

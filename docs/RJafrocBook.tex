...
\caption{\label{tab:ratingsParadigmExampleTable}Representative counts table.}
\centering
...
The spread appears to be more pronounced for the diseased cases, e.g., five of the 50 cases appeared to be definitely non-diseased to the observer. However, one is forewarned not to jump to conclusions about the spread of the data being larger for diseased than for non-diseased cases based on observed rating alone. While it turns out to be true as will be shown later, the \textbf{ratings are merely ordered labels}, and modeling is required, see Chapter \ref{BinormalModel}, that uses only the \emph{ordering information} implicit in the labels, not the \emph{actual values}, to reach quantitative conclusions.
...
\caption{\label{fig:ratingsParadigmEqVarFitA}(A): The left figure is the predicted ROC curve for \(\mu=1.282\) superposed on the operating points. (B): The right figure is the same data fitted with a two-parameter model described later.}
\end{figure}
...
\hypertarget{empiricalAUC}{%
\chapter{Empirical AUC}\label{empiricalAUC}}

\hypertarget{empiricalAUC-Intro}{%
\section{Introduction}\label{empiricalAUC-Intro}}
...
\begin{figure}
\centering
\includegraphics{05-empirical-auc_files/figure-latex/empiricalAUC-EmpiricalPlot-1.pdf}
\caption{\label{fig:empiricalAUC-EmpiricalPlot}Empirical ROC plot for the data in Table 4.1. By convention the operating points are numbered starting with the uppermost non-trivial one and working down the plot and the trivial operating points (0,0) and (1,1) are not shown.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{05-empirical-auc_files/figure-latex/empiricalAUC-EmpiricalAUC-1.pdf}
\caption{\label{fig:empiricalAUC-EmpiricalAUC}The empirical ROC plot corresponding to Table 4.1; the shaded area is the area AUC under this plot, a widely used figure of merit in non-parametric ROC analysis.}
\end{figure}
...
\label{eq:empiricalAUC-Wilcoxon}
\end{equation}
...
\begin{equation}
W = AUC
\label{eq:empiricalAUC-BambersTheorem}
\end{equation}

Numerical illustration: While hardly a proof, as an illustration of the theorem it is helpful to calculate the sum on the right hand side of Eqn. \eqref{eq:empiricalAUC-Wilcoxon} and compare it to direct integration of the area under the empirical ROC curve (i.e., adding the area of a triangle and several trapezoids). The function is called \texttt{trapz(x,y)}, see below. It takes two array arguments, \(x\) and \(y\), where in the current case \(x\) is \(FPF\) and \(y\) is \(TPF\). One has to be careful to include the end-points as otherwise the area will be underestimated. The Wilcoxon \(W\) and the numerical estimate of the empirical area AUC are implemented in the following code.
...
\begin{figure}
\centering
\includegraphics{05-empirical-auc_files/figure-latex/empiricalAUC-BambersTheorem-1.pdf}
\caption{\label{fig:empiricalAUC-BambersTheorem}:Illustration of the derivation of Bamber's equivalence theorem. Shows an empirical ROC plot for R = 5; the shaded area is due to points labeled i and i + 1.}
\end{figure}
...
\label{eq:empiricalAUC-BambersTheoremProof1}
\end{equation}
...
\label{eq:empiricalAUC-BambersTheoremProof2}
\end{equation}
...
\label{eq:empiricalAUC-BambersTheoremProof3}
\end{equation}
...
\label{eq:empiricalAUC-BambersTheoremProof4}
\end{equation}
...
\end{aligned}
\right \}
\label{eq:empiricalAUC-BambersTheoremProof5}
\end{equation}
...
\end{aligned}
\right \}
\label{eq:empiricalAUC-BambersTheoremProof6}
\end{equation}
...
Eqn. \eqref{eq:empiricalAUC-BambersTheoremProof6} can be restated as:

\begin{equation}
AUC=\frac{1}{2}P\left ( Z_1 = Z_2 \right ) + P\left ( Z_1 < Z_2 \right )
\label{eq:empiricalAUC-BambersTheoremProof7}
\end{equation}

The Wilcoxon statistic was defined in Eqn. \eqref{eq:empiricalAUC-Wilcoxon}. It can be seen that the comparisons implied by the summations and the weighting implied by the kernel function are estimating the two probabilities in the expression for in Eqn. \eqref{eq:empiricalAUC-BambersTheoremProof7}. Therefore, \(AUC = W\).
...
For \(i = 0\), Eqn. \eqref{eq:empiricalAUC-BambersTheoremProof5} implies (since the lowest empirical threshold is unity, the lowest allowed rating, and there are no cases rated less than one):
...
For \(i = 4\), Eqn. \eqref{eq:empiricalAUC-BambersTheoremProof5} implies:
...
\hypertarget{empiricalAUC-references}{%
\section{References}\label{empiricalAUC-references}}
...
\label{eq:BinormalModelZSampling1}
\end{equation}
...
\NormalTok{fArray \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.1}\NormalTok{,}\FloatTok{0.5}\NormalTok{,}\FloatTok{0.9}\NormalTok{)}
\NormalTok{seedArray \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{,}\DecValTok{11}\NormalTok{,}\DecValTok{12}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ (row }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{) \{ }
\NormalTok{  f \textless{}{-}}\StringTok{ }\NormalTok{fArray[row]}
\NormalTok{  seed \textless{}{-}}\StringTok{ }\NormalTok{seedArray[row]}
...
\NormalTok{  mu2 \textless{}{-}}\StringTok{ }\DecValTok{55}
\NormalTok{  sigma2 \textless{}{-}}\StringTok{ }\DecValTok{7} 
  \CommentTok{\# Simulate true gaussian ratings using above parameter values}
\NormalTok{  z1 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K1,}\DataTypeTok{mean =}\NormalTok{ mu1,}\DataTypeTok{sd =}\NormalTok{ sigma1)}
\NormalTok{  z1[z1}\OperatorTok{\textgreater{}}\DecValTok{100}\NormalTok{] \textless{}{-}}\StringTok{ }\DecValTok{100}\NormalTok{;z1[z1}\OperatorTok{\textless{}}\DecValTok{0}\NormalTok{] \textless{}{-}}\StringTok{ }\DecValTok{0} \CommentTok{\# constrain to 0 to 100}
\NormalTok{  z2 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K2,}\DataTypeTok{mean =}\NormalTok{ mu2,}\DataTypeTok{sd =}\NormalTok{ sigma2)}
\NormalTok{  z2[z2}\OperatorTok{\textgreater{}}\DecValTok{100}\NormalTok{] \textless{}{-}}\StringTok{ }\DecValTok{100}\NormalTok{;z2[z2}\OperatorTok{\textless{}}\DecValTok{0}\NormalTok{] \textless{}{-}}\StringTok{ }\DecValTok{0} \CommentTok{\# constrain to 0 to 100}
...
\NormalTok{  x \textless{}{-}}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x=}\NormalTok{Gaussians) }\CommentTok{\#  line 27}
\NormalTok{  x \textless{}{-}}\StringTok{  }
...
\NormalTok{  z \textless{}{-}}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\FloatTok{0.0}\NormalTok{, }\DecValTok{100}\NormalTok{, }\FloatTok{0.1}\NormalTok{)}
  \CommentTok{\# transform the latent Gaussians to true Gaussians}
\NormalTok{  transformation \textless{}{-}}\StringTok{ }
...
\StringTok{    }\KeywordTok{geom\_line}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ transformation, }\DataTypeTok{size =} \DecValTok{1}\NormalTok{) }\OperatorTok{+}
...
\NormalTok{  x \textless{}{-}}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x=}\NormalTok{y)}
\NormalTok{  x \textless{}{-}}\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ x, }\DataTypeTok{mapping =} \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x)) }\OperatorTok{+}
...
\label{eq:BinormalModelDemoMisconception}
\end{equation}
...
\label{eq:BinormalModelFPF}
\end{equation}

\begin{equation} 
TPF\left ( \zeta | \mu, \sigma \right ) = \Phi\left ( \frac{\mu-\zeta}{\sigma} \right )
\label{eq:BinormalModelTPF}
\end{equation}

These expressions allow calculation of the operating point for any \(\zeta\). An equation for a curve is usually expressed as \(y=f(x)\). An expression of this form for the ROC curve, i.e., the y coordinate (TPF) expressed as a function of the x coordinate (FPF), follows upon inversion of the expression for FPF, Eqn. \eqref{eq:BinormalModelFPF}:

\begin{equation} 
\zeta = -\Phi^{-1}\left ( FPF \right )
\label{eq:BinormalModelZeta}
\end{equation}

Substitution of Eqn. \eqref{eq:BinormalModelZeta} in Eqn. \eqref{eq:BinormalModelTPF} yields:

\begin{equation} 
TPF = \Phi\left ( \frac{\mu + \Phi^{-1}\left (FPF  \right )}{\sigma} \right )
\label{eq:BinormalModelRocCurve1}
\end{equation}
...
\label{eq:BinormalModelabParameters}
\end{equation}
...
By dividing the z-samples by \(b\), the variance of the distribution labeled ``Noise'' becomes unity, its mean stays at zero, and the variance of the distribution labeled ``Signal'' becomes \(1/b\), and its mean becomes \(a/b\), as shown below. It illustrates that the inverses of Eqn. \eqref{eq:BinormalModelabParameters} are:

\begin{equation} 
\mu=\frac{a}{b};\sigma=\frac{1}{b}
\label{eq:BinormalModelabParametersInv}
\end{equation}

Eqns. \eqref{eq:BinormalModelabParameters} and \eqref{eq:BinormalModelabParametersInv} allow conversion from one notation to another.
...
\begin{figure}
\centering
\includegraphics{06-binormal-model_files/figure-latex/BinormalModel-ab2-mu-sigma-1.pdf}
\caption{\label{fig:BinormalModel-ab2-mu-sigma}The left plot shows the definitions of the (a,b) parameters of the binormal model. In the right plot the x-axis has been rescaled so that the noise distribution has unit variance, thereby illustrations between (a,b) and the (\(\mu,\sigma\)) parameters.}
\end{figure}
...
Using the \((a,b)\) notation, Eqn. \eqref{eq:BinormalModelRocCurve1} for the ROC curve reduces to:

\begin{equation} 
TPF = \Phi\left ( a+ b \Phi^{-1}\left (FPF  \right ) \right )
\label{eq:BinormalModelRocCurve}
\end{equation}
...
From Eqn. \eqref{eq:BinormalModelFPF} and Eqn. \eqref{eq:BinormalModelTPF}, the expressions for \(FPF\) and \(TPF\) in terms of model parameters \((a,b)\) are:
...
\end{aligned}
\right \}
\label{eq:BinormalModelFPF-TPF-ab}
\end{equation}

\hypertarget{density-functions-pdfs-of-the-binormal-model}{%
\subsection{Density functions (pdfs) of the binormal model}\label{density-functions-pdfs-of-the-binormal-model}}

According to Eqn. \eqref{eq:BinormalModelZSampling1} the probability that a z-sample is smaller than a specified threshold \(\zeta\), i.e., the CDF function, is:
...
\hypertarget{fitting-an-roc-curve-to-data-points}{%
\section{Fitting an ROC curve to data points}\label{fitting-an-roc-curve-to-data-points}}

\hypertarget{a-java-fitted-roc-curve}{%
\subsection{A JAVA fitted ROC curve}\label{a-java-fitted-roc-curve}}

This section, described in the physical book, has been abbreviated to the \href{http://www.rad.jhmi.edu/jeng/javarad/roc/JROCFITi.html}{relevant website}.

\hypertarget{a-simplistic-straight-line-fit-to-the-roc-curve}{%
\subsection{A simplistic straight line fit to the ROC curve}\label{a-simplistic-straight-line-fit-to-the-roc-curve}}
...
By applying the function \(\Phi^{-1}\) to both sides of Eqn. \eqref{eq:BinormalModelRocCurve1}, one gets (the ``inverse'' function cancels the ``forward'' function on the right hand side):
...
\begin{figure}
\centering
\includegraphics{06-binormal-model_files/figure-latex/BinormalModelStraightLineFit-1.pdf}
\caption{\label{fig:BinormalModelStraightLineFit}The straight line fit method of estimating parameters of the fitting model.}
\end{figure}
...
\label{eq:BinormalModelProductProb}
\end{equation}
...
\label{eq:BinormalModelCombFactor}
\end{equation}

The likelihood function is the product of Eqn. \eqref{eq:BinormalModelProductProb} and Eqn. \eqref{eq:BinormalModelCombFactor}:
...
\CommentTok{\#\textgreater{} [1] 1.32045261}
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $b}
\CommentTok{\#\textgreater{} [1] 0.607492932}
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $zetas}
\CommentTok{\#\textgreater{}      zetaFwd1      zetaFwd2      zetaFwd3      zetaFwd4 }
\CommentTok{\#\textgreater{} 0.00768054675 0.89627306763 1.51564784976 2.39672209865 }
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $AUC}
\CommentTok{\#\textgreater{} [1] 0.870452157}
\CommentTok{\#\textgreater{} }
\CommentTok{\#\textgreater{} $StdAUC}
\CommentTok{\#\textgreater{}              [,1]}
\CommentTok{\#\textgreater{} [1,] 0.0379042262}
...
K_{tr} \geq 5
\label{eq:BinormalModelGoodnessFit}
\end{equation}
...
\end{split}
\label{eq:BinormalModelGoodnessFitExpVals}
\end{equation}
...
\begin{equation} 
C^2\sim \chi_{df}^{2}
\label{eq:BinormalModelGoodnessFitDistr}
\end{equation}
...
\label{eq:BinormalModelGoodnessFitdf}
\end{equation}
...
\label{eq:BinormalModelab2dprime}
\end{equation}
...
\hypertarget{sourcesVariability}{%
\chapter{Sources of AUC variability}\label{sourcesVariability}}

\hypertarget{sourcesVariabilityIntro}{%
\section{Introduction}\label{sourcesVariabilityIntro}}
...
\hypertarget{sourcesVariability3sources}{%
\section{Three sources of variability}\label{sourcesVariability3sources}}
...
\label{eq:sourcesVariability-cs-wr}
\end{equation}
...
\label{eq:sourcesVariability-br-wr}
\end{equation}
...
\label{eq:sourcesVariability-case-set}
\end{equation}
...
\hypertarget{sourcesVariabilityDeLong}{%
\section{DeLong method}\label{sourcesVariabilityDeLong}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bruteForceEstimation \textless{}{-}}
...
\NormalTok{    empAuc \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \DecValTok{10000}\NormalTok{)}
    \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(empAuc)) \{}
\NormalTok{      zk1 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K1)}
\NormalTok{      zk2 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K2, }\DataTypeTok{mean =}\NormalTok{ mu, }\DataTypeTok{sd =}\NormalTok{ sigma)}
\NormalTok{      empAuc[i] \textless{}{-}}\StringTok{ }\KeywordTok{Wilcoxon}\NormalTok{(zk1, zk2)}
\NormalTok{    \}}
\NormalTok{    stdDevempAuc  \textless{}{-}}\StringTok{  }\KeywordTok{sqrt}\NormalTok{(}\KeywordTok{var}\NormalTok{(empAuc))}
\NormalTok{    meanempAuc   \textless{}{-}}\StringTok{  }\KeywordTok{mean}\NormalTok{(empAuc)}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(}
...
\NormalTok{ret \textless{}{-}}\StringTok{ }\KeywordTok{bruteForceEstimation}\NormalTok{(seed, mu, sigma, K1, K2)}
\CommentTok{\# one more trial}
\NormalTok{zk1 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K1)}
\NormalTok{zk2 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K2, }\DataTypeTok{mean =}\NormalTok{ mu, }\DataTypeTok{sd =}\NormalTok{ sigma)}
\NormalTok{empAuc \textless{}{-}}\StringTok{ }\KeywordTok{Wilcoxon}\NormalTok{(zk1, zk2)}
\NormalTok{ret1  \textless{}{-}}\StringTok{ }\KeywordTok{DeLongVar}\NormalTok{(zk1,zk2)}
\NormalTok{stdDevDeLong \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(ret1)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{seed \textless{}{-}}\StringTok{ }\DecValTok{2}\NormalTok{;}\KeywordTok{set.seed}\NormalTok{(seed)}
\NormalTok{mu \textless{}{-}}\StringTok{ }\FloatTok{1.5}\NormalTok{;sigma \textless{}{-}}\StringTok{ }\FloatTok{1.3}\NormalTok{;K1 \textless{}{-}}\StringTok{ }\DecValTok{500}\NormalTok{;K2 \textless{}{-}}\StringTok{ }\DecValTok{520}
\NormalTok{ret \textless{}{-}}\StringTok{ }\KeywordTok{bruteForceEstimation}\NormalTok{(seed, mu, sigma, K1, K2)}
\CommentTok{\# one more trial}
\NormalTok{zk1 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K1)}
\NormalTok{zk2 \textless{}{-}}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(K2, }\DataTypeTok{mean =}\NormalTok{ mu, }\DataTypeTok{sd =}\NormalTok{ sigma)}
...
\item
  Notice also that the one sample empirical AUC for the smaller case-size is 0.863, which is less than two standard deviations from the population mean 0.819. The ``two standard deviations'' comes from rounding up 1.96: as in Eqn. \eqref{eq:binaryTask-def-z-alpha2}, where \(z_{\alpha/2}\) was defined as the upper \(1-\alpha/2\) quantile of the unit normal distribution and \(z_{0.025}=1.96\).
\item
...
\hypertarget{sourcesVariabilityBootstrap}{%
\section{Bootstrap method}\label{sourcesVariabilityBootstrap}}
...
\caption{\label{tab:sourcesVariabilitybs1}Representative counts table.}
\centering
\begin{tabular}[t]{l|r|r|r|r|r}
\hline
  & $r = 5$ & $r = 4$ & $r = 3$ & $r = 2$ & $r = 1$\\
...
\label{eq:sourcesVariabilityVar-bs}
\end{equation}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{doBootstrap \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(parametricFOM, B, seed, RocTable) \{}
  \CommentTok{\# this is the K vector }
\NormalTok{  K \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{sum}\NormalTok{(RocTable[}\DecValTok{1}\NormalTok{,]), }\KeywordTok{sum}\NormalTok{(RocTable[}\DecValTok{2}\NormalTok{,]))}
  
  \ControlFlowTok{if}\NormalTok{ (parametricFOM) \{}
\NormalTok{    ret \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocTable)                 }
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{    ret \textless{}{-}}\StringTok{ }\KeywordTok{WilcoxonCountsTable}\NormalTok{(RocTable) }
\NormalTok{  \}}
\NormalTok{  OrigAUC  \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{AUC}
...
\NormalTok{  z1  \textless{}{-}}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(RocTable[}\DecValTok{1}\NormalTok{,]),}
\NormalTok{             RocTable[}\DecValTok{1}\NormalTok{,])}
\NormalTok{  z2  \textless{}{-}}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(RocTable[}\DecValTok{2}\NormalTok{,]),}
\NormalTok{             RocTable[}\DecValTok{2}\NormalTok{,])}\CommentTok{\#do:}
\NormalTok{  AUC \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ B)}\CommentTok{\#to save the bs AUC values}
  \ControlFlowTok{for}\NormalTok{ ( b }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{B)\{}
    \ControlFlowTok{while}\NormalTok{ (}\DecValTok{1}\NormalTok{) \{}
\NormalTok{      RocTable\_bs  \textless{}{-}}\StringTok{ }
...
\NormalTok{      k1\_b \textless{}{-}}\StringTok{ }\KeywordTok{ceiling}\NormalTok{( }\KeywordTok{runif}\NormalTok{( K[ }\DecValTok{1}\NormalTok{ ] ) }\OperatorTok{*}\StringTok{ }\NormalTok{K[ }\DecValTok{1}\NormalTok{ ] )}
      \CommentTok{\# bs indices for diseased}
\NormalTok{      k2\_b \textless{}{-}}\StringTok{ }\KeywordTok{ceiling}\NormalTok{( }\KeywordTok{runif}\NormalTok{( K[ }\DecValTok{2}\NormalTok{ ] ) }\OperatorTok{*}\StringTok{ }\NormalTok{K[ }\DecValTok{2}\NormalTok{ ] )}
\NormalTok{      bsTable \textless{}{-}}\StringTok{ }\KeywordTok{table}\NormalTok{(z1[k1\_b])}
      \CommentTok{\#convert array to frequency table}
\NormalTok{      RocTable\_bs[}\DecValTok{1}\NormalTok{,}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{names}\NormalTok{(bsTable))] \textless{}{-}}\StringTok{ }
\StringTok{        }\NormalTok{bsTable}
\NormalTok{      bsTable \textless{}{-}}\StringTok{ }\KeywordTok{table}\NormalTok{(z2[k2\_b])}
      \CommentTok{\#do:}
\NormalTok{      RocTable\_bs[}\DecValTok{2}\NormalTok{,}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{names}\NormalTok{(bsTable))] \textless{}{-}}\StringTok{ }
\StringTok{        }\NormalTok{bsTable}
      \CommentTok{\#replace NAs with zeroes}
\NormalTok{      RocTable\_bs[}\KeywordTok{is.na}\NormalTok{(RocTable\_bs )] \textless{}{-}}\StringTok{ }\DecValTok{0}
      \ControlFlowTok{if}\NormalTok{ (parametricFOM) \{}
\NormalTok{        temp \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocTable\_bs)}
\NormalTok{      \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{        temp \textless{}{-}}\StringTok{ }\KeywordTok{WilcoxonCountsTable}\NormalTok{(RocTable\_bs) }
\NormalTok{      \}    }
\NormalTok{      AUC[b]  \textless{}{-}}\StringTok{ }\NormalTok{temp}\OperatorTok{$}\NormalTok{AUC}
...
\NormalTok{    \}}
\NormalTok{  \}}
\NormalTok{  meanAUCboot \textless{}{-}}\StringTok{ }\KeywordTok{mean}\NormalTok{(AUC)}
\NormalTok{  Var \textless{}{-}}\StringTok{ }\KeywordTok{var}\NormalTok{(AUC)}
\NormalTok{  stdAUCboot  \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(Var)}
  \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{parametricFOM \textless{}{-}}\StringTok{ }\OtherTok{TRUE}
\NormalTok{B \textless{}{-}}\StringTok{ }\DecValTok{200}\NormalTok{;seed \textless{}{-}}\StringTok{ }\DecValTok{1}\NormalTok{;}\KeywordTok{set.seed}\NormalTok{(seed)}
...
\NormalTok{RocTable[}\DecValTok{1}\NormalTok{,]  \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{30}\NormalTok{,}\DecValTok{19}\NormalTok{,}\DecValTok{8}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{)}
\NormalTok{RocTable[}\DecValTok{2}\NormalTok{,]  \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{,}\DecValTok{6}\NormalTok{,}\DecValTok{5}\NormalTok{,}\DecValTok{12}\NormalTok{,}\DecValTok{22}\NormalTok{)}

\NormalTok{ret \textless{}{-}}\StringTok{ }\KeywordTok{doBootstrap}\NormalTok{(parametricFOM, B, seed, RocTable)}
\NormalTok{OrigAUC \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{OrigAUC}
\NormalTok{meanAUCboot \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{meanAUCboot}
\NormalTok{stdAUCboot \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{stdAUCboot}
...
\hypertarget{sourcesVariabilityJackknife}{%
\section{Jackknife method}\label{sourcesVariabilityJackknife}}
...
\label{eq:sourcesVariabilityAUC-jackknife-fom}
\end{equation}
...
\label{eq:sourcesVariability-pseudovalues-Y}
\end{equation}

Here AUC denotes the estimate using the entire dataset, i.e., not removing any cases. The jackknife pseudovalues will turn out to be of central importance in TBA Chapter 09. The \emph{jackknife AUC values}, defined by Eqn. \eqref{eq:sourcesVariabilityAUC-jackknife-fom}, should not be confused with jackknife derived psuedovalues, defined by Eqn. \eqref{eq:sourcesVariability-pseudovalues-Y}.
...
\label{eq:sourcesVariability-jackknife-variance-definition}
\end{equation}
...
\label{eq:sourcesVariability-variance-definition}
\end{equation}
...
\NormalTok{  z1  \textless{}{-}}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(RocTable[}\DecValTok{1}\NormalTok{,]),}
\NormalTok{             RocTable[}\DecValTok{1}\NormalTok{,])}
\NormalTok{  z2  \textless{}{-}}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(RocTable[}\DecValTok{1}\NormalTok{,]),}
\NormalTok{             RocTable[}\DecValTok{2}\NormalTok{,])}
  
\NormalTok{  AUC\_jack  \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{sum}\NormalTok{(K))}
\NormalTok{  Y\_k \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{sum}\NormalTok{(K))}
\NormalTok{  z\_jk  \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{sum}\NormalTok{(K))}
...
\NormalTok{    RocTable\_jk  \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\KeywordTok{length}\NormalTok{(RocTable[}\DecValTok{1}\NormalTok{,])))}
    \ControlFlowTok{if}\NormalTok{ ( k }\OperatorTok{\textless{}=}\StringTok{ }\NormalTok{K[ }\DecValTok{1}\NormalTok{ ])\{}
\NormalTok{      z1\_jk \textless{}{-}}\StringTok{ }\NormalTok{z1[ }\OperatorTok{{-}}\NormalTok{k ]}
\NormalTok{      z2\_jk \textless{}{-}}\StringTok{ }\NormalTok{z2}
\NormalTok{    \}}\ControlFlowTok{else}\NormalTok{\{}
\NormalTok{      z1\_jk \textless{}{-}}\StringTok{ }\NormalTok{z1}
\NormalTok{      z2\_jk \textless{}{-}}\StringTok{ }\NormalTok{z2[ }\OperatorTok{{-}}\NormalTok{(k }\OperatorTok{{-}}\StringTok{ }\NormalTok{K[ }\DecValTok{1}\NormalTok{ ]) ] }
\NormalTok{    \}  }
    \CommentTok{\#convert array to frequency table}
\NormalTok{    RocTable\_jk[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(}\KeywordTok{table}\NormalTok{(z1\_jk))]  \textless{}{-}}\StringTok{ }
\StringTok{      }\KeywordTok{table}\NormalTok{(z1\_jk)}
\NormalTok{    RocTable\_jk[}\DecValTok{2}\NormalTok{,}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(}\KeywordTok{table}\NormalTok{(z2\_jk))]  \textless{}{-}}\StringTok{ }
\StringTok{      }\KeywordTok{table}\NormalTok{(z2\_jk)}
    \CommentTok{\#replace NAs with zeroes}
\NormalTok{    RocTable\_jk[}\KeywordTok{is.na}\NormalTok{(RocTable\_jk)] \textless{}{-}}\StringTok{ }\DecValTok{0}
    \CommentTok{\# AUC\_jack for observed data}
    \ControlFlowTok{if}\NormalTok{ (parametricFOM) \{}
\NormalTok{      temp \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocTable\_jk) }
\NormalTok{    \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{      temp \textless{}{-}}\StringTok{ }\KeywordTok{WilcoxonCountsTable}\NormalTok{(RocTable\_jk) }
\NormalTok{    \}      }
\NormalTok{    AUC\_jack[k]  \textless{}{-}}\StringTok{ }\NormalTok{temp}\OperatorTok{$}\NormalTok{AUC}
\NormalTok{    Y\_k[k] \textless{}{-}}\StringTok{ }\KeywordTok{sum}\NormalTok{(K)}\OperatorTok{*}\NormalTok{OrigAUC }\OperatorTok{{-}}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(K)}\OperatorTok{{-}}\DecValTok{1}\NormalTok{)}\OperatorTok{*}\NormalTok{AUC\_jack[k]}
    \ControlFlowTok{if}\NormalTok{ (AUC\_jack[k] }\OperatorTok{==}\StringTok{ }\DecValTok{{-}1}\NormalTok{) }
      \KeywordTok{stop}\NormalTok{(}\StringTok{"RocfitR did not converge in jackknife loop"}\NormalTok{) }
\NormalTok{  \}}
\NormalTok{  meanAUCjack \textless{}{-}}\StringTok{ }\KeywordTok{mean}\NormalTok{(AUC\_jack)}
  \CommentTok{\#Efron and Stein\textquotesingle{}s paper, include jackknife inflation factor}
\NormalTok{  Var\_jack \textless{}{-}}\StringTok{ }\KeywordTok{var}\NormalTok{(AUC\_jack) }\OperatorTok{*}\StringTok{ }\NormalTok{( }\KeywordTok{sum}\NormalTok{(K) }\OperatorTok{{-}}\StringTok{ }\DecValTok{1}\NormalTok{)}\OperatorTok{\^{}}\DecValTok{2} \OperatorTok{/}\StringTok{ }\KeywordTok{sum}\NormalTok{(K) }
\NormalTok{  stdAUCjack  \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(Var\_jack)}
  \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(}
    \DataTypeTok{OrigAUC =}\NormalTok{ OrigAUC,}
...
\NormalTok{ret \textless{}{-}}\StringTok{ }\KeywordTok{doJackknife}\NormalTok{(parametricFOM, RocTable)}
\NormalTok{OrigAUC \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{OrigAUC}
\NormalTok{meanAUCjack \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{meanAUCjack}
\NormalTok{stdAUCjack \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{stdAUCjack}
...
\hypertarget{sourcesVariabilityCalSimulator}{%
\section{Calibrated simulator}\label{sourcesVariabilityCalSimulator}}
...
As an example, consider a real clinical dataset, such as in Table \ref{tab:ratingsParadigmExampleTable}. This dataset, analyzed by MLE, yielded model parameters, \texttt{a}, \texttt{b} and the 4 thresholds \(\zeta_1,\zeta_2,\zeta_3,\zeta_4\). After conversion to \(\mu=a/b\) and \(\sigma= 1/b\) and new zetas \(\zeta = \zeta/b\), the values are (in the same order): 2.173597, 1.646099, 0.01263423, 1.475351, 2.494901, 3.945221 (see code output below):

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# x is the mu{-}sigma notation}
\NormalTok{x \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{2.173597}\NormalTok{, }\FloatTok{1.646099}\NormalTok{, }\FloatTok{0.01263423}\NormalTok{, }\FloatTok{1.475351}\NormalTok{, }\FloatTok{2.494901}\NormalTok{, }\FloatTok{3.945221}\NormalTok{)}
\CommentTok{\# y is the a{-}b notation}
\NormalTok{y \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.320453}\NormalTok{, }\FloatTok{0.607497}\NormalTok{, }\FloatTok{0.007675259}\NormalTok{, }\FloatTok{0.8962713}\NormalTok{, }\FloatTok{1.515645}\NormalTok{, }\FloatTok{2.39671}\NormalTok{)}
\NormalTok{y[}\DecValTok{1}\NormalTok{]}\OperatorTok{/}\NormalTok{y[}\DecValTok{2}\NormalTok{] }\CommentTok{\# this is mu}
\CommentTok{\#\textgreater{} [1] 2.173596}
\DecValTok{1}\OperatorTok{/}\NormalTok{y[}\DecValTok{2}\NormalTok{]    }\CommentTok{\# this is sigma}
\CommentTok{\#\textgreater{} [1] 1.646099}
\NormalTok{y[}\DecValTok{3}\OperatorTok{:}\DecValTok{6}\NormalTok{]}\OperatorTok{/}\NormalTok{y[}\DecValTok{2}\NormalTok{] }\CommentTok{\# this is zeta in mu{-}sigma notation}
...
On each pass through the simulator one samples 60 values from the non-diseased distribution and 50 values from the diseased distribution, which producers a simulated ROC counts table like Table \ref{tab:ratingsParadigmExampleTable}. MLE on the ROC counts table yields \(A_z\). The process is repeated \(P = 2000\) (\(p\) is the population sampling index, ranging from 1 to \(P\)) and finally one calculates the mean and standard deviation of the 2000 \(A_z\) values. Open the file mainCalSimulator.R, Online Appendix 7.C, confirm that FOM is set to ``Az'' at line 11 (i.e., it is not commented out) and source it. There is no \texttt{seed} dependence of the code (the physical book is incorrect on this point):

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/SimulateRocCountsTable.R"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

The following code implements function \texttt{doCalSimulator(P,\ parametricFOM,\ RocCountsTable)}. Here \texttt{P} is the number of population samples to perform, \texttt{parametricFOM} is a logical, if set to \texttt{TRUE}, the binormal model is used to calculate fitted AUC and otherwise the Wilcoxon statistic is used to calculate empirical AUC, and \texttt{RocCountsTable} contains the ROC data, such as \ref{tab:ratingsParadigmExampleTable}, to which the simulator is to be calibrated to.

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{doCalSimulator \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(P, parametricFOM, RocCountsTable) \{}
\NormalTok{  K \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{sum}\NormalTok{(RocCountsTable[}\DecValTok{1}\NormalTok{,]), }
         \KeywordTok{sum}\NormalTok{(RocCountsTable[}\DecValTok{2}\NormalTok{,]))}
  \CommentTok{\# to build the model we have to do a parametric fit first}
\NormalTok{  ret \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocCountsTable) }\CommentTok{\# AUC for observed data}
\NormalTok{  a \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{a}
\NormalTok{  b  \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{b}
\NormalTok{  zetas \textless{}{-}}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{zeta}
\NormalTok{  mu  \textless{}{-}}\StringTok{ }\NormalTok{a}\OperatorTok{/}\NormalTok{b}
\NormalTok{  sigma \textless{}{-}}\StringTok{ }\DecValTok{1}\OperatorTok{/}\NormalTok{b}
\NormalTok{  zetas \textless{}{-}}\StringTok{ }\NormalTok{zetas}\OperatorTok{/}\NormalTok{b }\CommentTok{\# need to also scale zetas}
   \CommentTok{\# AUC for observed data}
  \ControlFlowTok{if}\NormalTok{ (parametricFOM) \{}
\NormalTok{    OrigAUC \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocCountsTable)}\OperatorTok{$}\NormalTok{AUC                 }
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{    OrigAUC \textless{}{-}}\StringTok{ }\KeywordTok{WilcoxonCountsTable}\NormalTok{(RocCountsTable)}\OperatorTok{$}\NormalTok{AUC }
\NormalTok{  \}}
  \CommentTok{\#to save the pop sample AUC values}
\NormalTok{  AUC \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ P)}
  \ControlFlowTok{for}\NormalTok{ ( p }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{P)\{}
    \ControlFlowTok{while}\NormalTok{ (}\DecValTok{1}\NormalTok{) \{}
\NormalTok{      RocCountsTableSimPop \textless{}{-}}\StringTok{ }
\StringTok{        }\KeywordTok{SimulateRocCountsTable}\NormalTok{(K, mu, sigma, zetas)}
      \CommentTok{\#replace NAs with zeroes}
\NormalTok{      RocCountsTableSimPop[}\KeywordTok{is.na}\NormalTok{(RocCountsTableSimPop )] \textless{}{-}}\StringTok{ }\DecValTok{0}
      \ControlFlowTok{if}\NormalTok{ (parametricFOM) \{}
        \CommentTok{\# AUC for observed data }
\NormalTok{        temp \textless{}{-}}\StringTok{ }\KeywordTok{RocfitR}\NormalTok{(RocCountsTableSimPop)}
...
\NormalTok{          AUC[p]  \textless{}{-}}\StringTok{ }\NormalTok{temp}\OperatorTok{$}\NormalTok{AUC}
          \ControlFlowTok{break} 
\NormalTok{        \}}
\NormalTok{      \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{        AUC[p] \textless{}{-}}\StringTok{ }\NormalTok{(}\KeywordTok{WilcoxonCountsTable}\NormalTok{(RocCountsTableSimPop))}\OperatorTok{$}\NormalTok{AUC}
        \ControlFlowTok{break} 
\NormalTok{      \}    }
\NormalTok{    \}}
\NormalTok{  \}}
\NormalTok{  AUC \textless{}{-}}\StringTok{ }\NormalTok{AUC[}\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(AUC)]}
\NormalTok{  meanAUC  \textless{}{-}}\StringTok{ }\KeywordTok{mean}\NormalTok{(AUC)}
\NormalTok{  stdAUC  \textless{}{-}}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\KeywordTok{var}\NormalTok{(AUC))}
  \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(}
    \DataTypeTok{mu =}\NormalTok{ mu,}
    \DataTypeTok{sigma =}\NormalTok{ sigma,}
    \DataTypeTok{zetas =}\NormalTok{ zetas,}
    \DataTypeTok{OrigAUC =}\NormalTok{ OrigAUC,}
...
\begin{Highlighting}[]
\NormalTok{parametricFOM \textless{}{-}}\StringTok{ }\OtherTok{TRUE}
\NormalTok{P \textless{}{-}}\StringTok{ }\DecValTok{2000}
\NormalTok{RocCountsTable =}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{5}\NormalTok{))}
\NormalTok{RocCountsTable[}\DecValTok{1}\NormalTok{,]  \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{30}\NormalTok{,}\DecValTok{19}\NormalTok{,}\DecValTok{8}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{)}
\NormalTok{RocCountsTable[}\DecValTok{2}\NormalTok{,]  \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{,}\DecValTok{6}\NormalTok{,}\DecValTok{5}\NormalTok{,}\DecValTok{12}\NormalTok{,}\DecValTok{22}\NormalTok{)}
\NormalTok{ret \textless{}{-}}\StringTok{ }\KeywordTok{doCalSimulator}\NormalTok{(P, parametricFOM, RocCountsTable)}
\KeywordTok{cat}\NormalTok{(}\StringTok{"parametricFOM = "}\NormalTok{, parametricFOM,}
  \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{Calibrated simulator values:}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{mu = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{mu, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{sigma = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{sigma, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{zetas = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{zetas, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} parametricFOM =  TRUE }
\CommentTok{\#\textgreater{} Calibrated simulator values:}
\CommentTok{\#\textgreater{}  }
\CommentTok{\#\textgreater{} mu =  2.173597 }
\CommentTok{\#\textgreater{} sigma =  1.646099 }
\CommentTok{\#\textgreater{} zetas =  0.01263423 1.475351 2.494901 3.945221}

\KeywordTok{cat}\NormalTok{(}\StringTok{"Simulations using calibrated simulator"}\NormalTok{,}
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{P = "}\NormalTok{, P, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{OrigAUC = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{OrigAUC, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{meanAUC = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{meanAUC, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{stdAUC = "}\NormalTok{, ret}\OperatorTok{$}\NormalTok{stdAUC, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} Simulations using calibrated simulator }
\CommentTok{\#\textgreater{} P =  2000 }
\CommentTok{\#\textgreater{} OrigAUC =  0.8704519 }
\CommentTok{\#\textgreater{} meanAUC =  0.8675359 }
\CommentTok{\#\textgreater{} stdAUC =  0.04064876}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{comparison-of-methods-of-estimating-variability}{%
\section{Comparison of methods of estimating variability}\label{comparison-of-methods-of-estimating-variability}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/GenerateCaseSamples.R"}\NormalTok{))}
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/VarPopSampling.R"}\NormalTok{))}
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/VarBootstrap.R"}\NormalTok{))}
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/VarJack.R"}\NormalTok{))}
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/Wilcoxon.R"}\NormalTok{))}
\KeywordTok{source}\NormalTok{(}\KeywordTok{here}\NormalTok{(}\StringTok{"R/CH07{-}Variability/VarDeLong.R"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# this takes a long time to run and the results are not used below}
\CommentTok{\# the summary tables are hard{-}coded}
\NormalTok{FinalParameters \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.320455}\NormalTok{, }
                     \FloatTok{0.6074974}\NormalTok{, }
                     \FloatTok{0.007676989}\NormalTok{, }
                     \FloatTok{0.8962713}\NormalTok{, }
                     \FloatTok{1.515645}\NormalTok{, }
                     \FloatTok{2.396711}\NormalTok{)}
\NormalTok{a  \textless{}{-}}\StringTok{ }\NormalTok{FinalParameters[}\DecValTok{1}\NormalTok{]}
\NormalTok{b  \textless{}{-}}\StringTok{  }\NormalTok{FinalParameters[}\DecValTok{2}\NormalTok{]}
\NormalTok{zetas \textless{}{-}}\StringTok{ }\NormalTok{FinalParameters[}\DecValTok{3}\OperatorTok{:}\KeywordTok{length}\NormalTok{((FinalParameters))]}
\NormalTok{mu \textless{}{-}}\StringTok{ }\NormalTok{a}\OperatorTok{/}\NormalTok{b}
\NormalTok{sigma \textless{}{-}}\StringTok{ }\DecValTok{1}\OperatorTok{/}\NormalTok{b}
\NormalTok{K \textless{}{-}}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{60}\NormalTok{, }\DecValTok{50}\NormalTok{)}
\NormalTok{seed \textless{}{-}}\StringTok{ }\DecValTok{1}
\KeywordTok{cat}\NormalTok{(}\StringTok{"seed = "}\NormalTok{, seed, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{K1 = "}\NormalTok{, K[}\DecValTok{1}\NormalTok{], }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{K2 = "}\NormalTok{, K[}\DecValTok{2}\NormalTok{], }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{mu = "}\NormalTok{, mu, }
    \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{sigma = "}\NormalTok{, sigma, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} seed =  1 }
\CommentTok{\#\textgreater{} K1 =  60 }
\CommentTok{\#\textgreater{} K2 =  50 }
\CommentTok{\#\textgreater{} mu =  2.173598 }
\CommentTok{\#\textgreater{} sigma =  1.646098}

\NormalTok{P \textless{}{-}}\StringTok{ }\DecValTok{2000}
\NormalTok{B \textless{}{-}}\StringTok{ }\DecValTok{2000}
\NormalTok{P1 \textless{}{-}}\StringTok{ }\DecValTok{20}

\KeywordTok{set.seed}\NormalTok{( seed )}
\NormalTok{VPS\textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ P1)}
\ControlFlowTok{for}\NormalTok{ (p }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{P1) VPS[p] \textless{}{-}}\StringTok{ }\KeywordTok{VarPopSampling}\NormalTok{(K, mu, sigma, zetas, P)}

\KeywordTok{set.seed}\NormalTok{( seed )}
\NormalTok{VBS\textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ P1)}
\ControlFlowTok{for}\NormalTok{ (p }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{P1) VBS[p] \textless{}{-}}\StringTok{ }\KeywordTok{VarBootstrap}\NormalTok{(K, mu, sigma, zetas, B)}

\KeywordTok{set.seed}\NormalTok{( seed )}
\NormalTok{VJK\textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ P1)}
\ControlFlowTok{for}\NormalTok{ (p }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{P1) VJK[p] \textless{}{-}}\StringTok{ }\KeywordTok{VarJack}\NormalTok{(K, mu, sigma, zetas)}

\KeywordTok{set.seed}\NormalTok{( seed )}
\NormalTok{VDL \textless{}{-}}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =}\NormalTok{ P1)}
\ControlFlowTok{for}\NormalTok{ (p }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{P1) VDL[p] \textless{}{-}}\StringTok{ }\KeywordTok{VarDeLong}\NormalTok{(K, mu, sigma, zetas)}

\KeywordTok{cat}\NormalTok{(}\StringTok{"Mean Sd Pop Sampling = "}\NormalTok{, }\KeywordTok{mean}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(VPS)),}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} Mean Sd Pop Sampling =  0.03594515}
\KeywordTok{cat}\NormalTok{(}\StringTok{"Mean Sd Boot Sampling = "}\NormalTok{, }\KeywordTok{mean}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(VBS)),}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} Mean Sd Boot Sampling =  0.03656486}
\KeywordTok{cat}\NormalTok{(}\StringTok{"Mean Sd Jack Sampling = "}\NormalTok{, }\KeywordTok{mean}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(VJK)),}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} Mean Sd Jack Sampling =  0.03349281}
\KeywordTok{cat}\NormalTok{(}\StringTok{"Mean Sd DeLong  = "}\NormalTok{, }\KeywordTok{mean}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(VDL)),}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\CommentTok{\#\textgreater{} Mean Sd DeLong  =  0.03331918}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:sourcesVariabilityTableCompareA}Different estimates of standard deviation of AUC (* the entry for the DeLong method was obtained using the empirical AUC).}
\centering
\begin{tabular}[t]{l|l|l|l}
\hline
Dataset & AUC est. method & Var. est. method & std. dev. AUC\\
\hline
ExampleTable & Rocfit & MLE & 0.0378\\
\hline
ExampleTable & Rocfit & DeLong & *0.0380\\
\hline
ExampleTable & Rocfit & Bootstrap & 0.0438\\
\hline
ExampleTable & Rocfit & Jackknife & 0.0386\\
\hline
ExampleTable & Rocfit & PopulationSampling & 0.0403\\
\hline
\end{tabular}
\end{table}
...
\caption{\label{tab:sourcesVariabilityTableCompareB}Different estimates of standard deviation of AUC.}
\centering
\begin{tabular}[t]{l|l|l|l}
\hline
Dataset & AUC est. method & Var. est. method & std. dev. AUC\\
\hline
ExampleTable & Wilcoxon & MLE & NA\\
\hline
ExampleTable & Wilcoxon & DeLong & 0.0380\\
\hline
ExampleTable & Wilcoxon & Bootstrap & 0.0413\\
\hline
ExampleTable & Wilcoxon & Jackknife & 0.0369\\
\hline
ExampleTable & Wilcoxon & PopulationSampling & 0.0369\\
\hline
\end{tabular}
\end{table}
...
\caption{\label{tab:sourcesVariabilityTableCompareC}Different estimates of standard deviation of AUC.}
\centering
\begin{tabular}[t]{l|l|l|l}
\hline
Dataset & AUC est. method & Var. est. method & std. dev. AUC\\
\hline
Cal. Simulator 60 cases & Wilcoxon & MLE & NA\\
\hline
Cal. Simulator 60 cases & Wilcoxon & DeLong & 0.0333\\
\hline
Cal. Simulator 60 cases & Wilcoxon & Bootstrap & 0.0366\\
\hline
Cal. Simulator 60 cases & Wilcoxon & Jackknife & 0.0335\\
\hline
Cal. Simulator 60 cases & Wilcoxon & PopulationSampling & 0.0359\\
\hline
\end{tabular}
\end{table}
...
\caption{\label{tab:sourcesVariabilityTableCompareD}Different estimates of standard deviation of AUC.}
\centering
\begin{tabular}[t]{l|l|l|l}
\hline
Dataset & AUC est. method & Var. est. method & std. dev. AUC\\
\hline
Cal. Simulator 600 cases & Wilcoxon & MLE & NA\\
\hline
Cal. Simulator 600 cases & Wilcoxon & DeLong & 0.0113\\
\hline
Cal. Simulator 600 cases & Wilcoxon & Bootstrap & 0.0110\\
\hline
Cal. Simulator 600 cases & Wilcoxon & Jackknife & 0.0113\\
\hline
Cal. Simulator 600 cases & Wilcoxon & PopulationSampling & 0.0113\\
\hline
\end{tabular}
\end{table}

\hypertarget{sourcesVariabilityreaderExpertise}{%
\section{Dependence of AUC on reader expertise}\label{sourcesVariabilityreaderExpertise}}
...
7.8.1: Code Output
\textgreater{} source(`\textasciitilde/book2/02 A ROC analysis/A7 Sources of variability in AUC/software/mainBetweenReaderSd.R')
...
\hypertarget{sourcesVariabilityModalityEffect}{%
\section{Dependence of AUC on modality}\label{sourcesVariabilityModalityEffect}}

Suppose one conducts an ROC study with j (j =1,2,\ldots J) readers but there are I (i=1,2,\ldots I) modalities. This is frequently referred to as the multiple reader multiple case (MRMC) paradigm. Each reader interprets the same case sample, i.e., the same set of cases, in two or more modalities. Here is an example, in file MainModalityEffect.R. This file loads the Van Dyke dataset, consisting of two modalities and five readers described in Online Chapter 24. Source the code file to get:
7.9.1: Code Output
\textgreater{} source(`\textasciitilde/book2/02 A ROC analysis/A7 Sources of variability in AUC/software/mainModalityEffect.R')
...
\hypertarget{sourcesVariabilityBinning}{%
\section{Effect of binning}\label{sourcesVariabilityBinning}}
...
\tightlist
\item
  The second effect is varying numbers of thresholds or bins between the readers. One could be a radiologist, capable of maintaining at most about 6 bins, and the other an algorithmic observer, such as CAD, capable of maintaining more bins. Moreover, if the radiologist is an expert, the data points will tend to cluster near the initial near vertical part of the ROC (see Chapter 17 for explanation). This is illustrated using code in file mainBinVariability.R. Sourcing this code yields Fig. 7.1 (E -- F) and the AUC values shown in these plots.
\end{enumerate}
...
\hypertarget{sourcesVariabilityEmpiricalVsFitted}{%
\section{Empirical vs.~fitted AUCs}\label{sourcesVariabilityEmpiricalVsFitted}}
...
\hypertarget{sourcesVariability-Discussion}{%
\section{Discussion}\label{sourcesVariability-Discussion}}
...
\hypertarget{sourcesVariability-references}{%
\section{References}\label{sourcesVariability-references}}
...
\CommentTok{\#\textgreater{} 1    1.0968151          1   0.29496611}
\end{Highlighting}
\end{Shaded}

...
= 1.644854`). Compare 1.64 to the critical value  $-\Phi^{-1}\left ( {\alpha/2} \right)$ = 1.96 for a two-sided test. If the change is in the expected direction, it is more likely that one will reject the NH with a one-sided than with a two-sided test. The p-value for a one-sided test is given by: 
...
![](08-Hypothesis-Testing_files/figure-latex/unnamed-chunk-6-1.pdf)<!-- --> 

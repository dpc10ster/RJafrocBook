...
* The three variances, determined from the pilot study via Equation \@ref(eq:HBEqn4Pilot), are assumed to apply unchanged to the pivotal study (they are sample-size independent parameters of the DBM sampling model). 
...
* The starting point is Equation \@ref(eq:HBEqn4Pilot), repeated here except that *all quantities appearing in it, in particular the mean-squares, apply to the pivotal study*. 

\begin{equation}
df_2 =\frac{\left ( MSTR + \max \left (MSTC - MSTRC,0  \right ) \right )^2}{\text{MSTR}^2}(J-1)
(\#eq:Defdf2)
\end{equation}

The mean squares can be related to the variances by an equation analogous to Equation \@ref(eq:HBEqn4Pilot), except that all quantities in it apply to the pivotal study - as noted by the absence of asterisks. 
...
Substituting from Equation \@ref(eq:HBEqn4Pivotal) into Equation \@ref(eq:Defdf2) and dividing numerator and denominator by $K^2$, one has the final expression as coded in `RJafroc`:
...
FCrit <- qf(1 - alpha, 1, df2)
Power <- 1-pf(FCrit, 1, df2, ncp = deltaRRRC)
data.frame("J"= J,  "K" = K, "FCrit" = FCrit, "df2" = df2, "deltaRRRC" = deltaRRRC, "PowerRRRC" = Power)
#>    J   K     FCrit       df2 deltaRRRC  PowerRRRC
...
#RRRC
# J <- 10; K <- 163
# Delta <- (0.5*J*K*d^2)/(K*VarTR+max(J*VarTC,0)+VarEps)
...
# data.frame("J"= J,  "K" = K, "FCrit" = FCrit, "ddf" = ddf, "Delta" = Delta, "RRRCPower" = Power)

#FRRC
# J <- 10; K <- 133
# Delta <- (0.5*J*K*d^2)/(max(J*VarTC,0)+VarEps)
# ddf <- (K-1)
# FCrit <- qf(1 - alpha, 1, ddf)
# Power <- 1-pf(FCrit, 1, ddf, ncp = Delta)
# data.frame("J"= J,  "K" = K, "FCrit" = FCrit, "ddf" = ddf, "Delta" = Delta, "RRRCPower" = Power)
...
#RRFC
# J <- 10; K <- 53
# Delta <- (0.5*J*K*d^2)/(K*VarTR+VarEps)
# ddf <- (J-1)
# FCrit <- qf(1 - alpha, 1, ddf)
# Power <- 1-pf(FCrit, 1, ddf, ncp = Delta)
# data.frame("J"= J,  "K" = K, "FCrit" = FCrit, "ddf" = ddf, "Delta" = Delta, "RRRCPower" = Power)

...
CI_FRRC <- c(trtMeanDiffs - t_crit*stdErr, trtMeanDiffs + t_crit*stdErr)
names(CI_FRRC) <- c("Lower", "Upper")
print(CI_FRRC)
#>       Lower       Upper 
#> -0.08048591 -0.00711473
```

Note that this time the confidence interval does not include zero, consistent with the p-value finding.

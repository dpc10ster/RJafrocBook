...
```{r froc-meanings-theorems, fig.cap="An example wAFROC plot; from left to right, the two shaded areas correspond to $A_i$ and  $A_0$, respectively, defined below.", fig.show='hold', echo=FALSE}
print(p)
```
...
```{r froc-meanings-linearPlot-wafroc, fig.cap="Left: AFROC plot; right: corresponding wAFROC plot.", fig.show='hold', echo=FALSE}
...
```{r froc-meanings-table-non-diseased, echo=FALSE}
ndTable = array(dim = c(8,5))
ndTable[1,]  <- c("1", "1111", "NA",     "$-\\infty$", "\\*")
ndTable[2,]  <- c("1", "1121", "NA",     "$-\\infty$", "\\*")
ndTable[3,]  <- c("2", "2111", "0.487",  "0.487",      "F")
ndTable[4,]  <- c("2", "2121", "NA",     "0.487",      "F")
ndTable[5,]  <- c("3", "3111", "0.738",  "0.738",      "E")
ndTable[6,]  <- c("3", "3121", "0.576",  "0.738",      "E")
ndTable[7,]  <- c("4", "4111", "-0.305", "-0.305",     "H")
ndTable[8,]  <- c("4", "4121", "NA",     "-0.305",     "H")
df <- as.data.frame(ndTable)
colnames(df) <- c("case\\#", "$k_t t l_s s$", "$z_{k_t t l_s s}$", "$FP_{k_t t}$", "Label")
knitr::kable(df, caption = "Layout of mark-rating  pairs on non-diseased cases.", escape = FALSE)
```
...
```{r froc-meanings-table-diseased, echo=FALSE}
disTable = array(dim = c(8,8))
disTable[1,]  <- c("1", "1", "1211", "1.512", "1212", "0.852",  "1", "D")
disTable[2,]  <- c("1", "1", "1221", "NA",    "1222", "NA",     "NA", "")
disTable[3,]  <- c("2", "1", "2211", "NA",    "2212", "-0.215", "1", "G")
disTable[4,]  <- c("2", "1", "2221", "NA",    "2222", "NA",     "NA", "")

disTable[5,]  <- c("3", "2", "3211", "NA",    "3212", "1.588", "0.6", "C")
disTable[6,]  <- c("3", "2", "3221", "NA",    "3222", "NA",    "0.4", "\\*")
disTable[7,]  <- c("4", "2", "4211", "NA",    "4212", "2.944", "0.4", "A")
disTable[8,]  <- c("4", "2", "4221", "NA",    "4222", "1.984", "0.6", "B")
df <- as.data.frame(disTable)
colnames(df) <- c("case\\#",
                  "$L_{k_2}$",
                  "$k_t t l_s s$", 
                  "$z_{k_t t l_s s}$", 
                  ...
                  "$W_{k_2 l_2}$", 
                  "Label")
knitr::kable(df, caption = "Layout of mark-rating pairs on diseased cases.", escape = FALSE)
```
...
maxNL <- dim(frocDataRaw$ratings$NL)[4]
FP <- apply(frocDataRaw$ratings$NL, 3, max)
FP <- FP[1:K1]
LL <- frocDataRaw$ratings$LL[(frocDataRaw$lesions$weights != -Inf)]
ratings <- c(FP,	LL)
...
linearPlotAFROC <- ggplot(data = ret) + ggtitle ("A") + 
  geom_point(aes(x = ratings, y = pos, color = factor(truth)), size = 8) + 
  ...
  geom_text(data = z_text, aes(x = x, y = y, label = label), vjust = -0.5, hjust = 1, size = 5) + 
...
frocDataRaw$lesions$weights[3, ] <- c(0.6, 0.4)
frocDataRaw$lesions$weights[4, ] <- c(0.4, 0.6)
weight <- c(rep(1, length(FP)), frocDataRaw$lesions$weights[frocDataRaw$lesions$weights != -Inf])
ret <- data.frame(ratings = ratings, truth = truth, weight = weight, pos = pos)
ret$ratings[ret$ratings == -Inf] <- -0.9
z_text <- data.frame(x = 3.5, y = -0.09, label = "z")

linearPlotwAFROC <- ggplot(data = ret) + ggtitle("B") + 
  geom_point(aes(x = ratings, y = pos, color = factor(truth), size = factor(weight))) + 
  scale_color_manual(values = c("green", "red")) +
  scale_size_manual(values = c(3, 4, 5) + 3) + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(), legend.position="none") + 
  scale_y_continuous(name = "", breaks = NULL) + scale_x_continuous(name = "", breaks = NULL) + 
  geom_segment(aes(x = -1, y = -0.05, xend = 3.4, yend = -0.05), arrow = arrow(length = unit(0.3, "cm")), size = 1) +
  ...
  annotation_custom(grob = textGrob(bquote(z[3222]),
                    ...
                    xmin = -0.68, xmax = -0.68, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[32]==0.4),
                                    gp = gpar(fontsize = 8)), 
                    xmin = -0.68, xmax = -0.68, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = -0.06, y = -0.25, xend = -0.18, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[2212]),
                                    gp = gpar(fontsize = 8)), 
                    xmin = -0.05, xmax = -0.05, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[21]==1),
                                    gp = gpar(fontsize = 8)), 
                    xmin = -0.05, xmax = -0.05, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 1, y = -0.25, xend = 0.88, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[1212]),
                    ...
                    xmin = 1, xmax = 1, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[11]==1),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 1, xmax = 1, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 1.43, y = -0.25, xend = 1.55, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[3212]),
                    ...
                    xmin = 1.43, xmax = 1.43, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[31]==0.6),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 1.43, xmax = 1.43, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 2.12, y = -0.25, xend = 2, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[4222]),
                    ...
                    xmin = 2.12, xmax = 2.12, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[42]==0.6),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 2.12, xmax = 2.12, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 2.78, y = -0.25, xend = 2.9, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[4212]),
                    ...
                    xmin = 2.78, xmax = 2.78, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[41]==0.4),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 2.78, xmax = 2.78, 
                    ymin = -0.36, ymax = -0.36)
linearPlotwAFROC <- ggplotGrob(linearPlotwAFROC)
linearPlotwAFROC$layout$clip[linearPlotwAFROC$layout$name=="panel"] <- "off"
```

```{r froc-meanings-linearPlot-afroc, fig.cap="Plot A (AFROC generation) a one-dimensional depiction of the data in Table \\@ref(tab:froc-meanings-table-non-diseased) and Table \\@ref(tab:froc-meanings-table-diseased), showing z-samples used for plotting the AFROC; the red circles correspond to latent lesion localizations (LLs) and the green to latent false positives (FPs). Plot B (wAFROC generation): Data in same tables but this time including the weights, for plotting the weighted-AFROC plot; the sizes of the red circles code the lesions weights; the weights are shown below each z-sample.", fig.show='hold', echo=FALSE}
grid.arrange(linearPlotAFROC, linearPlotwAFROC, nrow = 2)
```
...
afrocPlot <- ggplot(AFROC_Points, aes(x = FPF, y = LLF) ) + ggtitle("A") + 
  geom_line(size = 1) + 
  geom_point(size = 4) + 
  ...
  scale_y_continuous(expand = c(0, 0), breaks = c(0.17, 0.33, 0.5, 0.67, 0.83)) +
...
wAFROC_Points <- data.frame(FPF = FPF, wLLF = wLLF)
wafrocPlot <- ggplot(wAFROC_Points, aes(x = FPF, y = wLLF) ) + geom_line(size = 1) +  ggtitle("B") +
  geom_point(size = 4) + 
  annotation_custom(
    grob = 
      textGrob(bquote(italic("A")),
               gp = gpar(fontsize = 8)), 
    xmin = 0.03, xmax = 0.03, 
    ymin = 0.13, ymax = 0.13) +
  annotation_custom(grob = textGrob(bquote(italic("B")),

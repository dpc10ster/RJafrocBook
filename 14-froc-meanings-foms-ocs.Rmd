...
```{r froc-meanings-numerical-illustrations, echo = FALSE, cache=TRUE}
K1 <- 4;K2 <- 4
...
* The second non-diseased case has one latent mark rated `r frocData$ratings$NL[1,1,2,1]`.
* The third non-diseased case has two latent marks rated `r frocData$ratings$NL[1,1,3,1]` and `r frocData$ratings$NL[1,1,3,2]`.
* The fourth non-diseased case has one latent mark rated `r frocData$ratings$NL[1,1,4,1]`.
* The first diseased case has one latent NL mark rated `r frocData$ratings$NL[1,1,5,1]`.
...
ndTable[1,]  <- c("", "1111", "",     "", "")
ndTable[2,]  <- c("1", "1121", "",     "$-\\infty$", "\\*")
ndTable[3,]  <- c("", "2111", "0.487",  "",      "")
ndTable[4,]  <- c("2", "2121", "",     "0.487",      "F")
ndTable[5,]  <- c("", "3111", "0.738",  "",      "")
ndTable[6,]  <- c("3", "3121", "0.576",  "0.738",      "E")
ndTable[7,]  <- c("", "4111", "-0.305", "",     "")
ndTable[8,]  <- c("4", "4121", "",     "-0.305",     "H")
df <- as.data.frame(ndTable)
colnames(df) <- c("", "$k_t t l_s s$", "$z_{k_t t l_s s}$", "$FP_{k_t t}$", "$\\text{Label}$")
knitr::kable(df, caption = "Layout of mark-rating  pairs on non-diseased cases; the asterisk denotes an unmarked non-diseased case.", escape = FALSE)
```
...
disTable = array(dim = c(8,9))
disTable[1,]  <- c("", "", "1211", "1.512", "1212", "0.852",  "", "", "")
disTable[2,]  <- c("1", "1", "1221", "",  "1222", "",     "1.512", "1", "D")
disTable[3,]  <- c("", "", "2211", "",    "2212", "-0.215", "", "", "")
disTable[4,]  <- c("2", "1", "2221", "",  "2222", "", "-0.215", "1",  "G")

disTable[5,]  <- c("", "", "3211", "",    "3212", "1.588", "1.588", "0.6", "C")
disTable[6,]  <- c("3", "2", "3221", "",  "3222", "",   "", "0.4", "\\*")
disTable[7,]  <- c("", "", "4211", "",    "4212", "2.944", "2.944", "0.4", "A")
disTable[8,]  <- c("4", "2", "4221", "",  "4222", "1.984", "1.984", "0.6", "B")
...
colnames(df) <- c("",
                  ...
                  "$\\text{weights}$",
                  "$\\text{Label}$")
knitr::kable(df, caption = "Layout of mark-rating pairs on diseased cases; the asterisk denotes an unmarked lesion.", escape = FALSE)
```

...
```{r standalone-cad-table-cells, echo=FALSE}
...
```{r rrfc-1t-auc, cache=TRUE, echo=TRUE}
...
```{r do_one_RRFC_1T}
do_one_RRFC_1T <- function(x,fom,method){
...
ret <- c(
  ...
  ciDif, # ci DIFF
  as.character(format(as.numeric(x$Tstat)^2, digits = dig)),  
  as.character(format(as.numeric(x$df), digits = dig)),  
  as.character(format(as.numeric(x$pval), digits = dig)))  
...
```{r do_one_RRRC_1T}
do_one_RRRC_1T <- function(x,fom,method){

dig <- 4
ciRad <- paste0("(", as.character(format(x$CIAvgRad[1], digits = 3)),
                ",",
                as.character(format(x$CIAvgRad[2], digits = 3)),")")
ciDif <- paste0("(", as.character(format(x$CIAvgDiffFom[1], digits = 3)),
                ",",
                as.character(format(x$CIAvgDiffFom[2], digits = 3)),")")
...
```{r do_one_RRRC_2T}
do_one_RRRC_2T <- function(x,fom,method){
ret_2T_RRRC_PCL_0_05$ciAvgRdrEachTrt$CILower
dig <- 4
ciRad <- paste0("(", as.character(format(x$ciAvgRdrEachTrt$CILower[1], digits = 3)),
                ",",
                as.character(format(x$CIAvgRad[2], digits = 3)),")")
ciDif <- paste0("(", as.character(format(x$CIAvgDiffFom[1], digits = 3)),
                ",",
...
ret <- c(
  ...
  as.character(format(x$avgDiffFom, digits = dig)),
  ciDif, # ci DIFF
  as.character(format(x$Fstat, digits = dig)),  
  as.character(format(x$ddf, digits = dig)),  
  as.character(format(x$pval, digits = dig)))  
...
# ret_1T_RRFC_PCL_0_05

allCells[1,1,] <- do_one_RRFC_1T (ret_1T_RRFC_PCL_0_05,"PCL_0_05","1T-RRFC")  
allCells[1,4,] <- do_one_RRFC_1T (ret_1T_RRFC_PCL_0_2,"PCL_0_2","1T-RRFC")  
allCells[1,7,] <- do_one_RRFC_1T (ret_1T_RRFC_PCL_1,"PCL_1","1T-RRFC")  
allCells[1,10,] <- do_one_RRFC_1T (ret_1T_RRFC_AUC,"AUC","1T-RRFC")  

allCells[1,2,] <- do_one_RRRC_2T (ret_2T_RRRC_PCL_0_05,"PCL_0_05","2T-RRRC")  
allCells[1,5,] <- do_one_RRRC_2T (ret_2T_RRRC_PCL_0_2,"PCL_0_2","2T-RRRC")  
allCells[1,8,] <- do_one_RRRC_2T (ret_2T_RRRC_PCL_1,"PCL_1","2T-RRRC")  
allCells[1,11,] <- do_one_RRRC_2T (ret_2T_RRRC_AUC,"AUC","2T-RRRC")  

allCells[1,3,] <- do_one_RRRC_1T (ret_1T_RRRC_PCL_0_05,"PCL_0_05","1T-RRRC")  
allCells[1,6,] <- do_one_RRRC_1T (ret_1T_RRRC_PCL_0_2,"PCL_0_2","1T-RRRC")  
allCells[1,9,] <- do_one_RRRC_1T (ret_1T_RRRC_PCL_1,"PCL_1","1T-RRRC")  
allCells[1,12,] <- do_one_RRRC_1T (ret_1T_RRRC_AUC,"AUC","1T-RRRC")  
...
1. Where a direct comparison is possible, namely $\text{1T-RRFC}$ analysis using and as FOMs, the p-values in Table \@ref(tab:standalone-cad-table-cells2) are similar to those reported in Study -- 1.
...
```{r standalone-cad-table-cells3, echo=FALSE}
cells = array(dim = c(12,8))
cells[1,]  <- c("PCL-0.05",    "RRFC",     "0.00819", rep(NA, 5))
cells[2,]  <- c("PCL-0.05", "2T-RRRC",           "0", "-0.00248",   "0.001", "0.00321",   "0.001",  "0.00978")
cells[3,]  <- c("PCL-0.05", "1T-RRRC",     "0.00819", "NA", "NA", "0.00442",      "NA", "0.01757")

cells[4,]  <- c( "PCL-0.2",    "RRFC",     "0.00282", rep(NA, 5))
cells[5,]  <- c( "PCL-0.2", "2T-RRRC",           "0",  "0.00017", "0.00075", "0.00231", "0.00075",  "0.00356")
cells[6,]  <- c( "PCL-0.2", "1T-RRRC",     "0.00282", "NA", "NA", "0.00313",      "NA", "0.00562")

cells[7,]  <- c(   "PCL-1",    "RRFC",     "0.00320", rep(NA, 5))
cells[8,]  <- c(   "PCL-1", "2T-RRRC",           "0",    "0.001", "0.00064", "0.00186", "0.00064",  "0.00246")
cells[9,]  <- c(   "PCL-1", "1T-RRRC",     "0.00320", "NA", "NA", "0.00244",      "NA", "0.00364")

cells[10,]  <- c(    "AUC",    "RRFC",     "0.00088", rep(NA, 5))
cells[11,]  <- c(    "AUC", "2T-RRRC",           "0",   "0.0002",  "0.0003",  "0.0007",  "0.0003",    "0.001")
cells[12,]  <- c(    "AUC", "1T-RRRC",     "0.00088", "NA", "NA",  "0.0009",      "NA",  "0.0014")

df <- as.data.frame(cells, stringsAsFactors = FALSE)
colnames(df) <- c("FOM", "Method", "$\\sigma_R^2$", "$\\sigma_{\\tau R}^2$", "Cov1", "Cov2", "Cov3", "Var")
kbl(df, caption = "Parameter estimates, NA = not applicable.", booktabs = TRUE, escape = FALSE) %>% collapse_rows(columns = c(1), valign = "middle") %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```

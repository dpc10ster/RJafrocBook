...
```{r ratingsParadigmEqVarFitA, fig.cap="The left figure is the predicted ROC curve for $\\mu=1.282$ superposed on the operating points. The right figure is the same data fitted with a two-parameter model described later.", fig.show='hold'}
grid.arrange(p1,p2,ncol=2)
```

It should come as no surprise that the uppermost operating point is *exactly* on the predicted curve: after all, this point was used to calculate $\mu$ = `r mu`. The corresponding value of $\zeta$ can be calculated from Eqn. (3.17), namely:
...
```{r SixPointScale, echo=FALSE, fig.cap="A method for acquiring ROC data on an effectively 6-point scale that also yields an unambiguous single operating point for declaring patients diseased. Note the reversal of the final ratings in the last \"column\" in the lower half of the figure.", fig.show='hold'}
knitr::include_graphics("images/AcquiringData6PointScale.png")
``` 
...
```{r 2AFC, echo=FALSE, fig.cap="Example of image presentation in a 2AFC study.", fig.show='hold'}
knitr::include_graphics("images/2AFC.png")
``` 
...
df[7,]  <- c("237",
              ...
              "839.66")
df <- as.data.frame(df)
colnames(df) <- c("Total number of mammograms",
                  "Mammograms without breast cancer (percent)",
                  "Mammograms with breast cancer  (percent)",
                  "Cancers per 1000 screening mammograms")
rownames(df) <- c("1:Normal",
                  "2: Benign finding",
                  "3: Probably benign, recommend normal or short term follow up",
                  "3-plus: Probably benign, recommend immediate follow up",
                  "0: Need additional imaging evaluation",
                  "4: Suspicious finding, biopsy should be considered",
                  "5: Highly suggestinve of malignancy")

...
```{r ratingsParadigmExampleTable, echo=FALSE}
Ktr = array(dim = c(2,5))
...
```{r ratingsParadigmEqVarFitA, fig.cap="(A): The left figure is the predicted ROC curve for $\\mu=1.282$ superposed on the operating points. (B): The right figure is the same data fitted with a two-parameter model described later.", fig.show='hold'}
...
* Fig. \@ref(fig:ratingsParadigmEqVarFitA) (B) shows the predicted ROC curve by the unequal variance binormal model, to be introduced in Chapter 06. The corresponding parameter values are $\mu$ = `r mu`and $\sigma$ = `r sigma`. 

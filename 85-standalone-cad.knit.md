...
```r
do_one <- function(x,fom,method){

dig <- 4
ciRad <- paste0("(", as.character(format(x$CIAvgRadFom[1], digits = 3)),
                ",",
                as.character(format(x$CIAvgRadFom[2], digits = 3)),")")
ciDif <- paste0("(", as.character(format(x$CIAvgDiffFom[1], digits = 3)),
                ",",
                as.character(format(x$CIAvgDiffFom[2], digits = 3)),")")

ret <- c(
  fom, 
  method,
  as.character(format(x$fomCAD, digits = dig)),
  "0", # ci CAD
  as.character(format(x$avgRadFom, digits = dig)),
  ciRad, # ci RAD
  as.character(format(x$avgDiffFom, digits = dig)),
  ciDif, # ci DIFF
  as.character(format(x$Tstat^2, digits = dig)),  
  as.character(format(x$df, digits = dig)),  
  as.character(format(x$pval, digits = dig)))  

...
```{r optim-op-point-AUC-vs-zeta1-10, fig.cap="Variation of AUC vs. $\\zeta_1$ for $\\lambda = 10$; AUC is the wAFROC AUC. panels are labeled by the value of $\\mu$ and zetaMax (i.e., $\\zeta_{\\text{max}}$, the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
...
do_one_froc <- function(mu, lambda, nu, zeta1, llfRange) {
  ...
  froc <- PlotRsmOperatingCharacteristics(
    ...
    zeta1, 
    lesDistr = lesDistr, 
    lesWghtDistr = UtilSpecifyLesionWeightsDistr(relWeights, relWeights), 
    OpChType = "FROC",
    llfRange = llfRange,
...
```{r optim-op-point-froc-10-first-two-plots, fig.cap="Extended FROC plots: panel labeled 10-1 is for $\\lambda = 10$ and $\\mu = 1$, and that labeled 10-1.5 is for $\\lambda = 10$ and $\\mu = 1.5$. The blue line indicates the optimal operating point.", fig.show='hold', echo=FALSE, fig.height=3.5,  fig.pos='H'}

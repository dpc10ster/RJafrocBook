...
The partial-area measure is $A_{z;c}$ = `r A_zc`. The value is small because the reporting threshold is high. However, $A_{z;c}$ is not be confused with true performance of the observer, as shown in Section TBA.
...
The binormal fitted AUC increases as $a$ increases or as $b$ decreases. Equivalently, it increases as $\mu$ increases or as $\sigma$ decreases. In the example just given, the full AUC is $A_z$ = `r A_z`.
...
Upper2 <- a/sqrt(1+b^2)
sigma <- rbind(c(1, rho), c(rho, 1))
pAuc <- array(dim = length(fpfArr))
for (i in 1:length(fpfArr)) {
  pAuc[i] <- as.numeric(pmvnorm(c(Lower1, Lower2), c(Upper1[i], Upper2), sigma = sigma))
  # add area under dashed line
  pAuc[i] <- pAuc[i] + (1 - fpfArr[i])*(1-tpfArr[i])/2 + (1-fpfArr[i]) * tpfArr[i]
}
df <- data.frame(FPF = fpfArr, pAUC_TRUE = pAuc)
...
```{r binormal-model-threshold-dependence-2, fig.cap="Variation of true performance pAUC_TRUE with FPF for a = 2 and b = 1. The curve levels out at pAUC_TRUE = $A_z$ = 0.9408251.", fig.show='hold', echo=FALSE}
grid.arrange(p2)
```
...
```{r binormal-model-threshold-dependence-3, fig.cap="The left panel shows the visibly improper ROC curve and now the solid line is not always above the dotted line. The right panel shows the variation of true performance pAUC_TRUE with FPF for a = 2 and b = 0.2. The curve exhibits a maximum at FPF approximately equal to 0.1.", fig.show='hold', echo=FALSE}
grid.arrange(p1, p2, ncol = 2)
```
...
```{r binormal-model-threshold-dependence-1, fig.cap="A binormal ROC curve corresponding to a = 2 and b = 1. The solid dot is the operating point corresponding to $\\zeta_1 = 1.5$ representing the upper limit of the partial ROC AUC. The dotted line connects the solid dot to (1,1). The solid curve extending from the origin to (1,1) represents the full ROC. Note that in the region above the solid dot the solid curve is above the dotted line, meaning true performance of an observer who only rates a sub-set of cases, those with z-samples greater than $\\zeta_1 = 1.5$, is less than performance of an observer who rates all cases.", fig.show='hold', echo=FALSE}
grid.arrange(p1)
```
...
```{r binormal-modelStraightLineFit, fig.cap = "The straight line fit method of estimating parameters of the fitting model.", fig.show='hold', echo=FALSE}

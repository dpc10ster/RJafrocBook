...
The area under the full curve is $A_z$ = `r Az`, the partial area measure is $A_{z;c}$ = `r pAuc`, while true performance of the partial area observer is $A_{z;c,\text{TRUE}}$ = `r pAucTrue`. For this example the major contribution to true performance comes from the area under the dotted straight line.
...
a <- 2;b <- 0.8
Az <-pnorm(a/sqrt(1+b^2))
fpf <- c(seq(0.0 , 0.0625, 0.001), seq(0.0625, 1, 0.01)) # FPF array
tpf <- pnorm(a + b * qnorm(fpf)) # TPF array
rho <- -b/sqrt(1+b^2)
Lower1 <- -Inf
Upper1 <- qnorm(fpf)
Lower2 <- -Inf
Upper2 <- a/sqrt(1+b^2)
sigma <- rbind(c(1, rho), c(rho, 1))
pAuc <- array(dim = length(fpf))
for (i in 1:length(fpf)) {
  pAuc[i] <- as.numeric(pmvnorm(c(Lower1, Lower2), c(Upper1[i], Upper2), sigma = sigma))
  # add area under dashed line
  pAuc[i] <- pAuc[i] + (1 - fpf[i])*(1-tpf[i])/2 + (1-fpf[i]) * tpf[i]
}
df <- data.frame(FPF = fpf, pAUC_TRUE = pAuc)
...
```{r binormal-model-threshold-dependence-2, fig.cap="Variation of true performance pAUC_TRUE with FPF for a = 2 and b = 0.8. The curve levels out at pAUC_TRUE = $A_z$ = 0.9408251.", fig.show='hold', echo=FALSE}
grid.arrange(p2)
```
...
```{r echo=FALSE}
a <- 2; b <- 0.2; zeta1Arr <- seq(1.5,5.5,0.05)

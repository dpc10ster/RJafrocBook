...
The partial-area measure is $A_{z;c}$ = `r A_zc`. The corresponding full-area measure is $A_z$ = `r A_z`. $A_{z;c}$ is small because the reporting threshold is high. However, $A_{z;c}$ should not be confused with true performance of the observer, as shown in Section \@ref(binormal-model-partial-true).
...
The d' index corresponding to the above binormal parameters is `r sqrt(2)*qnorm(A_z)`. The transformation from an index that ranges from 0.5 to 1 to one that ranges from 0 to infinity can be viewed as desirable. The d' index can be regarded as a perceptual signal-to-noise-ratio. 
...
```{r binormal-model-threshold-dependence-3, fig.cap="The left panel shows the visibly improper ROC curve and now the solid line is not always above the dotted line. The right panel shows the variation of true performance pAUC_TRUE with FPF for a = 1 and b = 0.2. The curve exhibits a maximum at FPF equal to 0.153.", fig.show='hold', echo=FALSE}
grid.arrange(p1, p2, ncol = 2)
```

...
The reason for the subscript "1" in $Cov_1$  will become clear when we extend this model to multiple readers. The $I \times I$ covariance matrix $\Sigma$  is: 
...
It is important to validate the significance testing method just outlined above. This is the subject of NH validation. If the testing procedure is correct, then, when the NH is true, it should reject it with probability $\alpha$. In the following, as is usual, we set $\alpha = 0.05$.


```{.r .numberLines}
set.seed(201)
mu <- 0.8
trueVar <- 0.0006989006
trueCov1 <- 0.0003734661
...
```
## True, estimated variances =  0.000699,   0.000695
```

```
## True, estimated Cov1 =  0.000373,   0.000351
```
...
Here is an application of the method for an ROC dataset, `dataset02`. 
...
Here is an application of the method for an FROC dataset, `dataset04`. 
...
The covariance matrix is estimated, as usual, by either a resampling method or, for the special case of Wilcoxon figure of merit, by the DeLong method.
...
It can be shown that under the null hypothesis that all treatments have identical performances, the test statistic $\chi_{1T}$ defined below (the $1T$ subscript denotes single-treatment analysis) is distributed approximately as a $\chi^2$ distribution with $J-1$ degrees of freedom, i.e., 

\begin{equation}
\chi_{\text{1T}} \equiv \frac{(J-1)MS(R)}{\text{Var}-\text{Cov2}} \sim \chi_{J-1}^{2}
(\#eq:chisq-single-reader)
\end{equation}


### p-value and confidence interval {#or-method-intro-pvalue-ci-1}
The p-value is the probability that a sample from the $\chi_{J-1}^{2}$ distribution is greater than the observed value of the test statistic, namely: 

\begin{equation}
p\equiv \Pr(\chi>\chi_{1T} \mid \chi \sim \chi_{J-1}^{2})
(\#eq:p-single-treatment)
\end{equation}



```{.r .numberLines}
# ds <- DfExtractDataset(dataset02, trts = 1)
# fom =  0.9196457 0.8587762 0.9038647 0.9731079 0.8297907
#ds <- DfExtractDataset(dataset02, trts = 1, rdrs = c(4,5))
#ds <- DfExtractDataset(dataset02, trts = 1)
ds <- DfExtractDataset(dataset02, trts = 1, rdrs = c(1,3))
fom <- as.numeric(UtilFigureOfMerit(ds, FOM = "Wilcoxon"))
vc <- UtilORVarComponentsFactorial(ds, FOM = "Wilcoxon")
cov2 <- vc$IndividualTrt$cov2EachTrt
var <- vc$IndividualTrt$varEachTrt
msR <- vc$IndividualTrt$msREachTrt
J <- length(ds$ratings$NL[1,,1,1])
chi1T <- (J-1)*msR/(var-cov2)
pval <- pchisq(chi1T,J-1,lower.tail = F)
ci <- array(dim = 2)
ci[1] <- (fom[1] - fom[2]) + qt(0.025, Inf, lower.tail = F) * sqrt(2*(var - cov2))
ci[2] <- (fom[1] - fom[2]) - qt(0.025, Inf, lower.tail = F) * sqrt(2*(var - cov2))
```
...
```
## fom =  0.9196457 0.9038647
```

```r
cat("diff_fom = ", fom[1]-fom[2], "\n")
```
...
```r
pv <- UtilPseudoValues(ds, FOM = "Wilcoxon")$jkPseudoValues
t.test(pv[1,1,], pv[1,2,], paired = T, var.equal = T)
```
...
## 	Paired t-test
## 
## data:  pv[1, 1, ] and pv[1, 2, ]
## t = 0.61043, df = 113, p-value = 0.5428
## alternative hypothesis: true difference in means is not equal to 0
## 95 percent confidence interval:
##  -0.03543706  0.06699905
## sample estimates:
...
Some elementary statistics are reviewed here.

...
Unlike the seemingly analogous single-reader multiple-treatment case, the single-treatment multiple-reader case is fundamentally different. This is because *reader is a random factor*. This makes it impossible to define a null hypothesis analogous to that with the treatment factor, e.g., $R_1 = R_2$ (since reader is modeled as a random sample from a distribution: $R \sim N(0,\sigma_R^2)$). The only statistics that can be estimated in this situation are the mean vector and the covariance matrix corresponding to the reader effect.

Significance testing between two readers can be performed if reader is regarded as a *fixed* effect. Then the analysis presented in Section \@ref(st-or-multiple-treatment) is applicable, with the treatment factor replaced by the reader factor. This is appropriate when comparing two AI (artificial intelligence) algorithms. The two algorithms, each of which qualifies as a reader, are not random samples from a population of AI readers: rather they are two fixed algorithms, in the very literal sense.

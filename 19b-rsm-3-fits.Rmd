...
With 14 datasets, comprising 43 modalities, 80 readers, 2012 cases, the total number of individual modality-reader combinations is 236: in other words, there are 236 datasets that each of the three algorithms was applied to. It is easy to be overwhelmed by the numbers and this section aims to summarize the most important conclusion: *all three fitting methods are consistent with a single method-independent AUC*.

```{r rsm-3-fits-inter-corr-scatter, echo=FALSE}
source(here("R/compare-3-fits/loadDataFile.R"))
source(here("R/compare-3-fits/interCorrScatter.R"))
```

```{r rsm-3-fits-inter-corr-scatter2, echo=FALSE}
ret <- interCorrScatter(type = "pearson", fileNames)
x <- cbind(ret$m_pro_rsm, ret$m_cbm_rsm)
x <- rbind(x, apply(x,2, mean))
x %>% mutate(across(is.numeric, ~ round(., 4))) -> x
row.names(x) <- c(as.character(1:14), "AVG")
colnames(x) <- c("m_pro_rsm", "R2_pro_rsm", "m_cbm_rsm", "R2_cbm_rsm")
```


```{r}
kbl(x, caption = "TBA.", booktabs = TRUE, escape = FALSE) %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```

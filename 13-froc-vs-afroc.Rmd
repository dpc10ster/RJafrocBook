...
```{r optOperatingPointLambda10, cache = TRUE, echo=TRUE}
...
```{r froc-vs-afroc-plot5, fig.cap="Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
...
```{r optOperatingPointLambda01, cache = TRUE, echo=FALSE}

lambda <- 1
nu <- 1
mu_arr <- c(1, 1.5, 2, 2.5)
maxFomArr_01 <- llfArr_01 <- nlfArr_01 <- array(dim = length(mu_arr))
plotArr <- array(list(), length(mu_arr))
K1 <- 5000
K2 <- 7000
Lmax <- 2
...
for (i in 1:length(mu_arr)) {
  ...
  physicalValues <- UtilIntrinsic2PhysicalRSM(mu_arr[i], lambda, nu)
  nlfArr_01[i] <- physicalValues$lambdaP*pnorm(-zetaMaxArr[2,i])
  llfArr_01[i] <- physicalValues$nuP*pnorm(mu_arr[i]-zetaMaxArr[2,i])
}
```



```{r froc-vs-afroc-plot6, fig.cap="Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]],plotArr[[2]],plotArr[[3]],plotArr[[4]], nrow = 2, ncol = 2)
```

```{r froc-vs-afroc-cad-optim-zeta1, echo=FALSE}
x1 <- as.data.frame(rbind(t(maxFomArr_10), t(nlfArr_10), t(llfArr_10)))
x2 <- as.data.frame(rbind(t(maxFomArr_01), t(nlfArr_01), t(llfArr_01)))
x12 <- rbind(x1, x2)
...
```{r froc-vs-afroc-cad-optim-table, echo=FALSE}
...
```{r optOpPntShowAllFrocs, cache = TRUE, echo=FALSE}
...
set.seed(seed)
Lk2 <- floor(runif(K2, 1, Lmax + 1))
pts <- array(dim = c(2, 2, 4)) # lambda, nlf/llf, 4-values of mu
pts[1,1,1] <- nlfArr_10[1]
pts[1,2,1] <- llfArr_10[1]
pts[1,1,2] <- nlfArr_10[2]
pts[1,2,2] <- llfArr_10[2]
pts[1,1,3] <- nlfArr_10[3]
pts[1,2,3] <- llfArr_10[3]
pts[1,1,4] <- nlfArr_10[4]
pts[1,2,4] <- llfArr_10[4]

pts[2,1,1] <- nlfArr_01[1]
pts[2,2,1] <- llfArr_01[1]
pts[2,1,2] <- nlfArr_01[2]
pts[2,2,2] <- llfArr_01[2]
pts[2,1,3] <- nlfArr_01[3]
pts[2,2,3] <- llfArr_01[3]
pts[2,1,4] <- nlfArr_01[4]
pts[2,2,4] <- llfArr_01[4]

extra <- array(dim = c(2, 4))
extra[1,] <- 0.1
extra[2,] <- 0.1
extra[2,2] <- 0.5
for (j in 1:2) {
  for (i in 1:length(mu_arr)) {
    ...
    vline <- data.frame(genAbscissa = NULL, genOrdinate = NULL)
    vline <- rbind(vline, data.frame(genAbscissa = pts[j,1,i], genOrdinate = 0))
    vline <- rbind(vline, data.frame(genAbscissa = pts[j,1,i], genOrdinate = pts[j,2,i]))
    
    x <- do_one_froc (mu_arr[i], 
                      lambdaArr[j], 
                      nu,
                      zetaMaxArr[j,i] - extra[j,i], # to keep vertical line in the range of the plot
                      K1, 
                      K2, 
                      Lk2, 
                      seed)
    plotArr[[j,i]] <- x$Plot + geom_line(data = vline, color = "red")  + 
      ggtitle(paste0(as.character(lambdaArr[j]),
                     "-",
...
```{r froc-vs-afroc-plot7a, fig.cap="TBA Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1,1]],
             plotArr[[1,2]], nrow = 1, ncol = 2)
```             



```{r froc-vs-afroc-plot7b, fig.cap="TBA Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1,3]],
             plotArr[[1,4]], nrow = 1, ncol = 2)
```             



```{r froc-vs-afroc-plot7c, fig.cap="TBA Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[2,1]],
             plotArr[[2,2]], nrow = 1, ncol = 2)
```             



```{r froc-vs-afroc-plot7d, fig.cap="TBA Variation of AUC vs. $\\zeta_1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[2,3]],
             plotArr[[2,4]], nrow = 1, ncol = 2)
```             

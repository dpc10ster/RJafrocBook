...
do_one_mu <- function(zeta1Arr, fomArray, mu, lambda, nu, K1, K2, Lk2, seed) {
  ...
  myData <- data.frame(zeta1 = zeta1Arr, fom = fomArray)
  p <- ggplot2::ggplot(myData, aes(x = zeta1, y = fom)) + geom_line()
...
```{r printLambda10, echo=FALSE}
x <- as.data.frame(rbind(t(max_fomArray), t(nlfArr), t(llfArr)))
colnames(x) <- paste0("mu = ", as.character(mu_arr))
rownames(x) <- c("maxFOM", "NLF", "LLF")
print(x)
```

```{r froc-vs-afroc-plot5, fig.cap="Variation of fom vs. $\\zeta_1$; fom is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes fom).", fig.show='hold', echo=FALSE}
...
```{r froc-vs-afroc-plot6, fig.cap="Variation of fom vs. $\\zeta_1$; fom is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes fom).", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]],plotArr[[2]],plotArr[[3]],plotArr[[4]], nrow = 2, ncol = 2)
```

...
```{r optOperatingPointLambda1, cache = TRUE, echo=FALSE}

lambda <- 1
nu <- 1
mu_arr <- c(1, 1.5, 2, 2.5)
maxFomArr_01 <- llfArr_01 <- nlfArr_01 <- zetaMaxArr <- array(dim = length(mu_arr))
plotArr <- array(list(), length(mu_arr))
K1 <- 5000
K2 <- 7000
Lmax <- 2
...
for (i in 1:length(mu_arr)) {
  ...
  physicalValues <- UtilIntrinsic2PhysicalRSM(mu_arr[i], lambda, nu)
  nlfArr_01[i] <- physicalValues$lambdaP*pnorm(-zetaMaxArr[i])
  llfArr_01[i] <- physicalValues$nuP*pnorm(mu_arr[i]-zetaMaxArr[i])
}
```
...
x1 <- cbind(c("AUC_10", "NLF_10", "LLF_10"), (as.data.frame(rbind(t(maxFomArr_10), t(nlfArr_10), t(llfArr_10)))))
colnames(x1) <- c("measure", paste0("mu = ", as.character(mu_arr)))
x2 <- cbind(c("AUC_01", "NLF_01", "LLF_01"), (as.data.frame(rbind(t(maxFomArr_01), t(nlfArr_01), t(llfArr_01)))))
colnames(x2) <- c("measure", paste0("mu = ", as.character(mu_arr)))
x <- rbind(x1, x2)
y <- c("AUC_10", "AUC_01", "NLF_10", "LLF_10", "NLF_01", "LLF_01")
x %>% slice(match(y, measure)) -> xFinal
```


```{r froc-vs-afroc-cad-optim-table, echo=FALSE}
knitr::kable(xFinal, digits = 5, caption = "Summary of CAD optimal threshold simulations: \"measure\" refers to a performance measure: AUC, NLF or LLF; the labels are as follows: AUC_10 is the wAFROC AUC for $\\lambda = 10$, AUC_01 is the wAFROC AUC for $\\lambda = 01$, NLF_10 is NLF for $\\lambda = 1$, ..., LLF_01 is LLF for $\\lambda = 1$.", escape = FALSE)
```

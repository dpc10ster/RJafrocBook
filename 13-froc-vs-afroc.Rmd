...
x <- wafroc_plot_2B$data$genAbscissa[wafroc_plot_2B$data$Reader == "R: RAD"];fpfRad2B <- x[length(x)-1]
x <- wafroc_plot_2D$data$genAbscissa[wafroc_plot_2D$data$Reader == "R: RAD"];fpfRad2D <- x[length(x)-1]
...
do_one_mu <- function(zeta1Arr, fomArray, mu, lambda, nu, K1, K2, Lk2, seed) {
  ...
  for (i in 1:length(zeta1Arr)) {
    ...
    fomArray[i] <- as.numeric(UtilFigureOfMerit(frocCad, FOM = "wAFROC"))
...
for (i in 1:length(mu_arr)) {
  ...
  frocCad <- SimulateFrocDataset (
    ...
    perCase = Lk2,
    seed = seed)
  max_fomArray[i] <- as.numeric(UtilFigureOfMerit(frocCad, FOM = "wAFROC"))

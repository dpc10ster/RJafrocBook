...
* `dataset14` "FEDERICA-REAL-ROC" Federical Zanca *real* ROC dataset [@RN2318]
...
## Application to 14 datasets {#rsm-3-fits-roc-application-14-datasets}
...
PROPROC is a two-parameter model, with parameters $c$ and $d_a$, as detailed in TBA Chapter 20: these parameters were obtained by running Windows software (OR DBM-MRMC 2.5 (Sept. 04, 2014) with PROPROC selected as the curve-fitting method. The relevant results are saved in files that end with "proprocnormareapooled.csv" contained in "R/compare-3-fits/MRMCRuns/xx/", where xx denotes the name of the dataset (for example, for the Van Dyke dataset, xx = "VD") and white-spaces in the original output file name have been removed. The `*.lrc` files are the data files suitably formatted for input to OR DBM-MRMC 2.5. The output files can be opened by `RStudio` and the values $c$ and $d_a$  are in the last two columns, see Fig. \@ref(fig:rsm-3-fits-proproc-output-van-dyke). The column names are T = treatment; R = reader; area = PROPROC AUC; numCAT = number of ROC bins; adjPMean = undocumented value; c =  $c$ and d_a =  $d_a$ parameters defined in [@RN1499].
...
Fig. \@ref(fig:rsm-3-fits-proproc-output-van-dyke) Contents of file `VDproprocareapooled.csv` generated by Windows DBM-MRMC for the `VD` dataset. Since there are five readers and two treatments, the file has 10 data rows. Row 7, showing results for modality 2 and reader 1, yields $c$ = 0.321 and $d_a$ = 2.35, and PROPROC AUC = 0.952. Row 10, for modality 2 and reader 4, yields $c$ = 1 and  $d_a$ = 0, corresponding to perfect performance, PROPROC AUC = 1. 
...
<div class="figure">
<img src="19b-rsm-3-fits_files/figure-html/rsm-3-fits-f2-1.png" alt="Variation of AUC vs. $\zeta_1$ for $\lambda = 1$." width="672" /><img src="19b-rsm-3-fits_files/figure-html/rsm-3-fits-f2-2.png" alt="Variation of AUC vs. $\zeta_1$ for $\lambda = 1$." width="672" /><img src="19b-rsm-3-fits_files/figure-html/rsm-3-fits-f2-3.png" alt="Variation of AUC vs. $\zeta_1$ for $\lambda = 1$." width="672" /><img src="19b-rsm-3-fits_files/figure-html/rsm-3-fits-f2-4.png" alt="Variation of AUC vs. $\zeta_1$ for $\lambda = 1$." width="672" /><img src="19b-rsm-3-fits_files/figure-html/rsm-3-fits-f2-5.png" alt="Variation of AUC vs. $\zeta_1$ for $\lambda = 1$." width="672" />
<p class="caption">(\#fig:rsm-3-fits-f2)Variation of AUC vs. $\zeta_1$ for $\lambda = 1$.</p>
</div>



 
Fig. 18.3: Screen shot of the MRMCRuns directory, opened using RStudio, showing the contents of the file VD_MRMC proproc area pooled.csv generated by Windows DBM-MRMC for the VD dataset with PROPROC analysis selected. The last two columns contain the  ,   parameters of the PROPROC algorithm for this dataset. For example, for modality 2 and reader 1,   =  0.321 and   = 2.35. Since there are five readers and two treatments, the file has 10 data rows. Row 10, showing results for modality 2 and reader 5, yielded c = 1 and  = 0, corresponding to perfect performance, AUC = 1. `T` = treatment; `R` = reader; `area` = PROPROC AUC; `numCAT` = # ROC bins; `adjPMean` = undocumented value; `c` =  $c$ parameter; `d_a` =  $d_a$ parameter defined in [@RN1499].
...
They are index by f (for filename). Line 24 selects the dataset to be analyzed. In the example shown the "TONY" dataset has been selected21. Line 27 - 28 loads the dataset (loadDataFile) and converts it to an ROC dataset (DfFRoc2HrRoc; for an ROC dataset this function is redundant). The PROPROC parameter values were extracted from the saved text files, described above, using function ProprocFits(), called at line 37. Line 55 implements Eqn. (36) of the Metz and Pan paper6 to calculate   from the retrieved  ,   parameters. The formula will be detailed later in Chapter 20. Line 56 calls function FitRsmRoc(frocData,i,j). The (i,j) indices tell the function which treatment and reader to analyze. The conversion to ROC data is performed internal to this function . Lines 57 – 60 extract the three parameters and the RSM-fitted AUC. Line 61 calls function UtilOperatingPoints() to get the ROC operating points defined by the selected dataset, modality and reader. Line 63 performs the CBM fit using function FitCbmRoc(). 

...
Is the new value, `r AUC`, sufficiently different from the population mean, `r meanAUC`, to reject the null hypothesis $NH: \text{AUC} = \text{AUC}_{pop}$? Note that the answer to this question can be either yes or no: equivocation is not allowed!
...
[The $Z$ notation here should not be confused with z-sample, decision variable or rating of a case in an ROC study; the latter, when sampled over a set of non-diseased and diseased cases, yield a realization of $\text{AUC}$. The author trusts the distinction will be clear from the context.] The observed magnitude of $z$ is `r z`. [Upper-case for random variable, lower-case for realized or observed value.]
...
The p-value corresponding to an observed $z$ of `r z` is given by:
...
cat("pop NH mean AUC = ", meanAUC, ", pop NH sigma AUC = ", sigmaAUC, "\n")
cat("num. non-diseased images = ", K1, "num. diseased images = ", K2, "\n")
cat("obsvdPower = ", ObsvdTypeIErrRate, "\n")
cat("95% confidence interval = ", CI, "\n")
cat("Effect Size = ", EffectSize(mu, sigma, muAH, sigma), "\n")
```
...
T <- 2000
alphaArray = c(0.05, 0.01)
for (i in 1:length(alphaArray)) {
  alpha <- alphaArray[i] 
  reject = array(0, dim = T)
  for (t in 1:length(reject)) {  
    zk1 <- rnorm(K1);zk2 <- rnorm(K2, mean = muAH, sd = sigma)  
  ...
  ObsvdTypeIErrRate <- sum(reject)/length(reject)
  cat("alpha = ", alpha, "obsvdPower = ", ObsvdTypeIErrRate, "\n")
}
```

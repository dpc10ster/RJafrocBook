...
```{r optim-op-point-plot5, fig.cap="Variation of AUC vs. $\\zeta_1$ for $\\lambda = 10$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
...
```{r optim-op-point-plot6, fig.cap="Variation of AUC vs. $\\zeta_1$ for $\\lambda = 1$; AUC is the wAFROC AUC. The plots are labeled by the value of $\\mu$ and zetaMax (the value of $\\zeta_1$ that maximizes AUC).", fig.show='hold', echo=FALSE}
...
knitr::kable(xFinal, digits = 5, caption = "Summary of CAD optimal threshold simulations: \"measure\" refers to a performance measure, \"mu1\" refers to $\\mu = 1$, etc. The row labels are as follows: AUC10 is the wAFROC AUC for lambda = 10, AUC01 is the wAFROC AUC for $\\lambda = 01$, NLF10 is NLF for $\\lambda = 1$, ..., LLF01 is LLF for $\\lambda = 1$.", escape = FALSE)
```

...
* The vertical red line is drawn at the *predicted* optimal NLF corresponding to $\zeta_{\text{max}}$ for this parameter combination, not the *empirical* NLF ^[Meaning of *extended*: due to sampling effects the predicted end-point will not, in general, coincide with the *empirical* end-point, but will be near it. In order to show a fuller extent of the FROC curve it is necessary to *extend* the observed curve beyond the predicted end-point. This was done by setting $\zeta_1$ = $\zeta_{\text{max}} - 0.5$, which has the effect of letting the curve run a little bit further to the right. As a concrete example the predicted end-point for the curve in question is (`r optOpPt[1,1,1]`, LLF = `r optOpPt[1,2,1]`) while the empirical end-point is (`r optOpPt1[1,1,1]`, LLF = `r optOpPt1[1,2,1]`) and the extended curve shown ends at (`r optOpPt2[1,1,1]`, LLF = `r optOpPt2[1,2,1]`).].  
* In Fig. \@ref(fig:optim-op-point-plot7a) AUC performance is quite low. In fact AUC = `r maxFomArr[1,1]`. The optimal operating point of the algorithm is actually rather close to the origin, specifically NLF = `r optOpPt[1,1,1]` and LLF = `r optOpPt[1,2,1]`.
...
* It remains to explain the seemingly anomalous behavior seen in the fifth row of Table \@ref(tab:optim-op-point-cad-optim-table) - NLF shows a peak as a function of $\mu$. The relevant FROC curves are shown in In Fig. \@ref(fig:optim-op-point-plot7b). The basic reason is that as $\mu$ increases the end-point of the FROC moves upwards and to the left (approaching NLF = 0 and LLF = 1 in the limit of infinite $\mu$). Because of this effect the expected increase in NLF seen in the third row of the table is terminated in the fifth row - one literally runs out of FROC curve to move up on. Another way of explaining this is that in In Fig. \@ref(fig:optim-op-point-plot7a) the true end-point, i.e., that corresponding to $\zeta_1 = -\infty$, is much further to the right than in Fig. \@ref(fig:optim-op-point-plot7b). This allows NLF to keep increasing as a function of $\mu$ in Fig. \@ref(fig:optim-op-point-plot7a) but not in Fig. \@ref(fig:optim-op-point-plot7b). 

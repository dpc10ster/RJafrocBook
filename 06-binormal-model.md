...
## The binormal model {#binormal-model-definition}
...
(\#eq:binormal-modelFPF)
\end{equation}

\begin{equation} 
TPF\left ( \zeta | \mu, \sigma \right ) = \Phi\left ( \frac{\mu-\zeta}{\sigma} \right )
(\#eq:binormal-modelTPF)
\end{equation}

These expressions allow calculation of the operating point for any $\zeta$. An equation for a curve is usually expressed as $y=f(x)$. An expression of this form for the ROC curve, i.e., the y coordinate (TPF) expressed as a function of the x coordinate (FPF), follows upon inversion of the expression for FPF, Eqn.  \@ref(eq:binormal-modelFPF):
...
Substitution of Eqn. \@ref(eq:binormal-modelZeta) in Eqn. \@ref(eq:binormal-modelTPF) yields:
...
From Eqn. \@ref(eq:binormal-modelFPF) and Eqn. \@ref(eq:binormal-modelTPF), the expressions for $FPF$ and $TPF$ in terms of model parameters $(a,b)$ are:
...
\end{aligned}
\right \}
(\#eq:binormal-modelFPF-TPF-ab)
\end{equation}

## Density functions (pdfs) of the binormal model {#binormal-model-pdf-curves}
According to Eqn. \@ref(eq:binormal-modelZSampling1) the probability that a z-sample is smaller than a specified threshold  $\zeta$, i.e., the CDF function, is:
...
$A_{z;c}$ is the area under the partial ROC curve extending from $FPF = 0$ to $FPF = c$ and $\phi\left ( z_1,z_2;\rho \right )$ is the standard bivariate normal distribution with correlation coefficient $\rho$ defined by:
...
## Partial-area measure vs. true performance {#binormal-model-partial-true}
A *partial area observer* is defined as follows: for the **sub-set of cases** defined by those with with z-samples exceeding $\zeta_1$ the observer reports ratings exactly equal to the observed z-samples (or some monotonic transformation of the z-samples), but for the remaining cases, i.e., with $z < \zeta_1$, the observer assigns a *fixed value rating that is smaller than $\zeta_1$* (the exact value does not matter). 

A distinction is made between the partial area measure $A_{z;c}$ and true performance. $A_{z;c}$ is determined by the portion of the ROC curve corresponding to $z \geq \zeta_1$. The ROC curve for such an observer is shown in \@ref(fig:binormal-model-threshold-dependence-1) for the following parameters: $a = 2$, $b = 0.8$ and $\zeta_1 = 1.5$. The last corresponds to $c \equiv FPF =  \Phi(-\zeta_1) = 0.0668$ and $TPF =  \Phi(a - b\zeta_1) = 0.788$. The ROC of the partial-area observer stops at the solid dot, i.e., at (0.0668, 0.788), as shown by *the solid curve ending at the solid dot*. The full ROC curve,  as shown by the complete solid curve, extends from (0,0) to (1,1). 

**To measure true performance one must, of course, include all cases.** True performance, for the current example, is measured by the area under the ROC curve from the origin to the solid dot **plus the area under the dotted line** extending from the solid dot to (1,1). The true performance of the partial area observer, denoted $A_{z;c,TRUE}$, is defined by:


\begin{equation} 
A_{z;c,\text{TRUE}} = A_{z;c} + \left ( 1 - FPF_{\text{max}} \right ) \left ( TPF_{\text{max}} \right ) + 0.5  \left ( 1 - FPF_{\text{max}} \right ) \left ( 1 - TPF_{\text{max}} \right )
(\#eq:binormal-model-partial-area-true-performance)
\end{equation}


One adds to $A_{z;c}$ the area under the dotted line, i.e, the area of a rectangle plus the area of a triangle. As should be clear from the plot, *true performance is smaller than $A_z$*. This is always true if the ROC curve is proper. For the selected parameter values the curve - which is actually improper for any $b \neq 1$ - is "quasi" proper in the sense that the eventual chance-line crossing near the upper right corner is invisible. The case of a visibly improper ROC curve will be considered shortly.
...
pAucTrue <- pAuc + (1 - fpf)*(1-tpf)/2 + (1-fpf) * tpf
```
...
The next figure, Fig. \@ref(fig:binormal-model-threshold-dependence-2) shows the variation of true performance $A_{z;c,\text{TRUE}}$ vs. FPF. 
...
The next figure shows the case of a visibly improper ROC curve, corresponding to $b = 0.2$ with other parameters unchanged. The left plot in Fig. \@ref(fig:binormal-model-threshold-dependence-3), illustrates the situation when the ROC curve is visibly improper. Now the dashed line is mostly above the continuous section and true performance is maximized at a finite value of $\zeta_1$, corresponding to $FPF \approx  0.1$, see right plot, an invalid conclusion due to the fact that an improper ROC curve is a fitting artifact of the binormal model easily avoided by using modern curve-fitting methods (PROPROC, CBM or RSM). However, this example serves a purpose, elaborated on in Chapter \@ref(optim-op-point), where it is shown that by using the area under the wAFROC - an operating characteristic that has an improper-like feature which is not a fitting artifact - one can find the optimal threshold of an algorithmic observer, such as CAD.
...
## Validating the fitting model {#binormal-model-curve-fitting-validation}
...
## Appendix I: Area under binormal ROC {#binormal-model-appendix-1}
...
## Appendix II: invariance of binormal model to arbitrary monotone transformations {#binormal-model-appendix-invariance-property}

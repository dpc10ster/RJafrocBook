...
Using the $(a,b)$ notation, Eqn. \@ref(eq:binormal-modelRocCurve1) for the ROC curve reduces to:

\begin{equation} 
TPF = \Phi\left ( a+ b \Phi^{-1}\left (FPF  \right ) \right )
(\#eq:binormal-modelRocCurve)
\end{equation}
...
By applying the function $\Phi^{-1}$  to both sides of Eqn. \@ref(eq:binormal-modelRocCurve1), one gets (the "inverse" function cancels the "forward" function on the right hand side):

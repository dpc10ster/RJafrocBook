...
frocNotation[10,]  <- c("10", "$\\zeta_r\\quad(r=2,3,...,R_{FROC})$", "Other reporting thresholds")
frocNotation[11,]  <- c("11", "$\\zeta_0, \\zeta_{R_{FROC}+1}$", "Dummy thresholds")
frocNotation[12,]  <- c("12", "$N_{k_t t}$", "Number of NLs on case $k_t t$")
frocNotation[13,]  <- c("13", "$L_{k_2}$", "Number of lesions on case $k_2 2$")
frocNotation[14,]  <- c("14", "$L_{max}$", "Maximum number of lesions per case in dataset")
frocNotation[15,]  <- c("15", "$L_T$", "Total number of lesions in dataset")
df <- as.data.frame(frocNotation)
colnames(df) <- c("Row number", "Symbol", "Meaning")
knitr::kable(df, caption = "FROC notation; all marks refer to latent marks; see footnotes", escape = FALSE)
```             
